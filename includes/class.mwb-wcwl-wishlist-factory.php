...
if ( ! class_exists( 'mwb_WCWL_Wishlist_Factory' ) ) {
	/**
	 * This class is used to create all Wishlist object required by the plugin
	 *
	 * @since 3.0.0
	 */
	class mwb_WCWL_Wishlist_Factory {
		/**
		 ...
		 * @return \mwb_WCWL_Wishlist|bool Wishlist object or false on failure
		 */
		public static function get_wishlist( $wishlist_id = false, $context = 'view' ) {
			...
			try {
				return new mwb_WCWL_Wishlist( $wishlist_id );
			} catch ( Exception $e ) {
		 ...
		 *              ]
		 *
		 * @return \mwb_WCWL_Wishlist[]|bool A list of matching wishlists or false on failure
		 */
		public static function get_wishlists( $args = array() ) {
			$args = apply_filters( 'mwb_wcwl_wishlist_query_args', $args );

			try {
				$results = WC_Data_Store::load( 'wishlist' )->query( $args );
				return apply_filters( 'mwb_wcwl_wishlist_query', $results, $args );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return false;
			}
		...
		public static function get_wishlists_count( $args = array() ) {
			$args = apply_filters( 'mwb_wcwl_wishlists_count_query_args', $args );

			try {
				$result = WC_Data_Store::load( 'wishlist' )->count( $args );
				return apply_filters( 'mwb_wcwl_wishlist_count_query', $result, $args );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return 0;
			}
		...
		public static function get_wishlist_users( $args = array() ) {
			$args = apply_filters( 'mwb_wcwl_wishlist_users_query_args', $args );

			try {
				$results = WC_Data_Store::load( 'wishlist' )->search_users( $args );
				return apply_filters( 'mwb_wcwl_wishlist_user_query', $results, $args );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return false;
			}
		...
		public static function get_current_wishlist( $args = array() ) {
			...
			if(
				empty( $action ) ||
				! in_array( $action, mwb_WCWL()->get_available_views() ) ||
				in_array( $action, array( 'view', 'user' ) ) ||
				( in_array( $action, array( 'manage', 'create' ) ) && ! mwb_WCWL()->is_multi_wishlist_enabled() )
			){
				switch( $action ){
					case 'user':
						$user_id = $value;
		...
		public static function get_default_wishlist( $id = false, $context = 'read' ) {
			try {
				...
				return apply_filters( 'mwb_wcwl_default_wishlist', $default_wishlist, $id, $context );
			} catch ( Exception $e ) {
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return false;
			}
		 ...
		 *
		 * @param $item_id int|\mwb_WCWL_Wishlist_Item|stdClass Item identifier, or item itself
		 * @return \mwb_WCWL_Wishlist_Item|bool Wishlist item, or false on failure
		 */
		public static function get_wishlist_item( $item_id = 0 ) {
			if ( is_numeric( $item_id ) ) {
				$id = $item_id;
			} elseif ( $item_id instanceof mwb_WCWL_Wishlist_Item ) {
				$id = $item_id->get_id();
			...
			if ( $id ) {
				try {
					return new mwb_WCWL_Wishlist_Item( $id );
				} catch ( Exception $e ) {
					return false;
				}
			}
		 ...
		 * @return mwb_WCWL_Wishlist_Item|bool Item, or false when no item found
		 */
		 ...
		 * ]
		 *
		 * @return \mwb_WCWL_Wishlist_Item[]|bool A list of matching items or false on failure
		 */
		public static function get_wishlist_items( $args = array() ) {
			$args = apply_filters( 'mwb_wcwl_wishlist_items_query_args', $args );

			try {
				$results = WC_Data_Store::load( 'wishlist-item' )->query( $args );
				return apply_filters( 'mwb_wcwl_wishlist_item_query', $results, $args );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return false;
			}
		...
		public static function get_wishlist_items_count( $args = array() ) {
			$args = apply_filters( 'mwb_wcwl_wishlist_items_count_query_args', $args );

			try {
				$result = WC_Data_Store::load( 'wishlist-item' )->count( $args );
				return apply_filters( 'mwb_wcwl_wishlist_item_count_query', $result, $args );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return 0;
			}
		...
		public static function get_times_added_count( $product_id ) {
			try {
				...
				return apply_filters( 'mwb_wcwl_wishlist_times_added_count_query', $result, $product_id );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return 0;
			}
		...
		public static function get_times_current_user_added_count( $product_id ) {
			try {
				...
				return apply_filters( 'mwb_wcwl_wishlist_times_current_user_added_count_query', $result, $product_id );
			} catch( Exception $e ){
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
				return 0;
			}
