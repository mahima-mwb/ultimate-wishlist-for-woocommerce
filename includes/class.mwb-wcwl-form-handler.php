...
if ( ! class_exists( 'mwb_WCWL_Form_Handler' ) ) {
	/**
	 * WooCommerce Wishlist Form Handler
	 *
	 * @since 3.0.0
	 */
	class mwb_WCWL_Form_Handler {
		/**
		...
		public static function init() {
			...
			add_action( 'init', array( 'mwb_WCWL_Form_Handler', 'add_to_wishlist' ) );

			// remove from wishlist when js is disabled.
			add_action( 'init', array( 'mwb_WCWL_Form_Handler', 'remove_from_wishlist' ) );

			// remove from wishlist after add to cart.
			add_action( 'woocommerce_add_to_cart', array( 'mwb_WCWL_Form_Handler', 'remove_from_wishlist_after_add_to_cart' ) );

			// change wishlist title.
			add_action( 'init', array( 'mwb_WCWL_Form_Handler', 'change_wishlist_title' ) );
		}
		...
		public static function process_form_handling() {
			...
			if ( $user_agent && apply_filters( 'mwb_wcwl_block_user_agent', preg_match( '/bot|crawl|slurp|spider|wordpress/i', $user_agent ), $user_agent ) ) {
				return false;
			}
		...
		public static function add_to_wishlist() {
			...
			if ( isset( $_GET['add_to_wishlist'] ) ) {
				try {
					...
					mwb_wcwl_add_notice( apply_filters( 'mwb_wcwl_product_added_to_wishlist_message', get_option( 'mwb_wcwl_product_added_text' ) ), 'success' );
				} catch ( Exception $e ) {
					mwb_wcwl_add_notice( apply_filters( 'mwb_wcwl_error_adding_to_wishlist_message', $e->getMessage() ), 'error' );
				}
			}
		}
		...
		public static function remove_from_wishlist() {
			...
			if ( isset( $_GET['remove_from_wishlist'] ) ) {
				try {
					...
					mwb_wcwl_add_notice( $e->getMessage(), 'error' );
				}
			}
		}
		...
		public static function remove_from_wishlist_after_add_to_cart() {
			if ( 'yes' != get_option( 'mwb_wcwl_remove_after_add_to_cart' ) ) {
				return;
			}
			...
			} elseif ( mwb_wcwl_is_wishlist() && isset( $_REQUEST['add-to-cart'] ) ) {
			...
			if ( ! empty( $args['wishlist_id'] ) ) {
				$wishlist = mwb_wcwl_get_wishlist( $args['wishlist_id'] );

				if ( $wishlist && $wishlist->is_current_user_owner() ) {
					try {
						mwb_WCWL()->remove( $args );
					} catch ( Exception $e ) {
		...
		public static function change_wishlist_title() {
			if ( ! isset( $_POST['mwb_wcwl_edit_wishlist'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mwb_wcwl_edit_wishlist'] ) ), 'mwb_wcwl_edit_wishlist_action' ) || ! isset( $_POST['save_title'] ) || empty( $_POST['wishlist_name'] ) ) {
				return;
			}
			...
			if ( ! $wishlist_name || strlen( $wishlist_name ) >= 65535 ) {
				mwb_wcwl_add_notice( __( 'Please, make sure to enter a valid title', 'mwb-woocommerce-wishlist' ), 'error' );
			} else {
