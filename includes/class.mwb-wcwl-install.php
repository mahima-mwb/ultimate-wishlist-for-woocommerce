...
if ( ! class_exists( 'mwb_WCWL_Install' ) ) {
	/**
	 * Install plugin table and create the wishlist page
	 *
	 * @since 1.0.0
	 */
	class mwb_WCWL_Install {

		/**
		 * Single instance of the class
		 *
		 * @var \mwb_WCWL_Install
		 * @since 2.0.0
		 */
		...
		public function __construct() {
			...
			$this->_table_items = $wpdb->prefix . 'mwb_wcwl';
			$this->_table_wishlists = $wpdb->prefix . 'mwb_wcwl_lists';

			// add custom field to global $wpdb.
			$wpdb->mwb_wcwl_items = $this->_table_items;
			$wpdb->mwb_wcwl_wishlists = $this->_table_wishlists;

			// define constant to use allover the application.
			define( 'mwb_WCWL_ITEMS_TABLE', $this->_table_items );
			define( 'mwb_WCWL_WISHLISTS_TABLE', $this->_table_wishlists );

			/**
			 * @deprecated
			 */
			define( 'mwb_WCWL_TABLE', $this->_table_items );
		}
		...
		public function register_current_version() {
			...
			update_option( 'mwb_wcwl_version', mwb_WCWL_VERSION );

			delete_option( 'mwb_wcwl_db_version' );
			update_option( 'mwb_wcwl_db_version', mwb_WCWL_DB_VERSION );
		}
		...
		private function _update_300() {
			...
			foreach ( $options as $option ) {
				$base_option_name = "mwb_wcwl_{$option}";
			...
			// duplicate options.
			$options = array(
				'mwb_wcwl_color_button_style_1' => array(
					'mwb_wcwl_color_ask_an_estimate',
				),
				'mwb_wcwl_color_button_style_1_hover' => array(
					'mwb_wcwl_color_ask_an_estimate_hover',
				),
				'woocommerce_promotion_mail_settings' => array(
					'woocommerce_mwb_wcwl_promotion_mail_settings',
				),
			);
			...
			$use_buttons = get_option( 'mwb_wcwl_use_button' );
			$use_theme_style = get_option( 'mwb_wcwl_frontend_css' );
			...
			update_option( 'mwb_wcwl_add_to_wishlist_style', $destination_value );
			update_option( 'mwb_wcwl_add_to_cart_style', $destination_value );
			update_option( 'mwb_wcwl_ask_an_estimate_style', $destination_value );

			// rounded corners options.
			$rounded_corners = get_option( 'mwb_wcwl_rounded_corners' );
			$radius_value = 'yes' == $rounded_corners ? 16 : 0;

			update_option( 'mwb_wcwl_rounded_corners_radius', $radius_value );
			update_option( 'mwb_wcwl_add_to_cart_rounded_corners_radius', $radius_value );
			update_option( 'mwb_wcwl_ask_an_estimate_rounded_corners_radius', $radius_value );
		}
		...
		private function _add_wishlists_table() {
			...
			if ( ! $this->is_installed() || version_compare( get_option( 'mwb_wcwl_db_version' ), '3.0.0', '<' ) ) {
		...
		private function _add_pages() {
			wc_create_page(
				sanitize_title_with_dashes( _x( 'wishlist', 'page_slug', 'mwb-woocommerce-wishlist' ) ),
				'mwb_wcwl_wishlist_page_id',
				__( 'Wishlist', 'mwb-woocommerce-wishlist' ),
				'<!-- wp:shortcode -->[mwb_wcwl_wishlist]<!-- /wp:shortcode -->'
			);
		}
	}
}
...
/**
 * Unique access to instance of mwb_WCWL_Install class
 *
 * @return \mwb_WCWL_Install
 * @since 2.0.0
 */
function mwb_WCWL_Install() {
	return mwb_WCWL_Install::get_instance();
}