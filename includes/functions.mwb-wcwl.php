...
if ( ! function_exists( 'mwb_wcwl_is_wishlist' ) ) {
	/**
	 ...
	 * @since 2.0.0
	 */
	function mwb_wcwl_is_wishlist() {
		global $mwb_wcwl_is_wishlist;

		return $mwb_wcwl_is_wishlist;
	}
}

if ( ! function_exists( 'mwb_wcwl_is_wishlist_page' ) ) {
	/**
	 ...
	 * @since 2.0.13
	 */
	function mwb_wcwl_is_wishlist_page() {
		...
		return apply_filters( 'mwb_wcwl_is_wishlist_page', is_page( $wishlist_page_id ) );
	}
}

if ( ! function_exists( 'mwb_wcwl_is_single' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	function mwb_wcwl_is_single() {
		return apply_filters( 'mwb_wcwl_is_single', is_product() && ! in_array( wc_get_loop_prop( 'name' ), array( 'related', 'up-sells' ) ) && ! wc_get_loop_prop( 'is_shortcode' ) );
	}
}

if ( ! function_exists( 'mwb_wcwl_is_mobile' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	function mwb_wcwl_is_mobile() {
		global $mwb_wcwl_is_mobile;

		return apply_filters( 'mwb_wcwl_is_wishlist_responsive', true ) && ( wp_is_mobile() || $mwb_wcwl_is_mobile );
	}
}

/* === TEMPLATE FUNCTIONS === */

if ( ! function_exists( 'mwb_wcwl_locate_template' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 */
	function mwb_wcwl_locate_template( $path, $var = null ) {
		$woocommerce_base = WC()->template_path();

		$template_woocommerce_path = $woocommerce_base . $path;
		$template_path             = '/' . $path;
		$plugin_path               = mwb_WCWL_DIR . 'templates/' . $path;
		...
		if ( ! $located && file_exists( $plugin_path ) ) {
			return apply_filters( 'mwb_wcwl_locate_template', $plugin_path, $path );
		}

		return apply_filters( 'mwb_wcwl_locate_template', $located, $path );
	}
}

if ( ! function_exists( 'mwb_wcwl_get_template' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 */
	function mwb_wcwl_get_template( $path, $var = null, $return = false ) {
		$located = mwb_wcwl_locate_template( $path, $var );
...
if ( ! function_exists( 'mwb_wcwl_get_template_part' ) ) {
	/**
	 ...
	 * @return string|null
	 */
	function mwb_wcwl_get_template_part( $template = '', $template_part = '', $template_layout = '', $var = array(), $return = false ) {
		...
		$template_hierarchy = apply_filters(
			'mwb_wcwl_template_part_hierarchy',
			array_merge(
				! mwb_wcwl_is_mobile() ? array() : array(
		...
		foreach ( $template_hierarchy as $filename ) {
			$located = mwb_wcwl_locate_template( $filename );

			if ( $located ) {
				return mwb_wcwl_get_template( $filename, $var, $return );
			}
		}
	}
}
...
if ( ! function_exists( 'mwb_wcwl_count_products' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 */
	function mwb_wcwl_count_products( $wishlist_token = false ) {
		return mwb_WCWL()->count_products( $wishlist_token );
	}
}

if ( ! function_exists( 'mwb_wcwl_count_all_products' ) ) {
	/**
	 ...
	 * @since 2.0.13
	 */
	function mwb_wcwl_count_all_products() {
		return mwb_WCWL()->count_all_products();
	}
}

if ( ! function_exists( 'mwb_wcwl_count_add_to_wishlist' ) ) {
	/**
	 ...
	 * @since 2.0.13
	 */
	function mwb_wcwl_count_add_to_wishlist( $product_id = false ) {
		return mwb_WCWL()->count_add_to_wishlist( $product_id );
	}
}

if ( ! function_exists( 'mwb_wcwl_get_count_text' ) ) {
	/**
	 ...
	 */
	function mwb_wcwl_get_count_text( $product_id = false ) {
		$count              = mwb_wcwl_count_add_to_wishlist( $product_id );
		$current_user_count = $count ? mwb_WCWL_Wishlist_Factory::get_times_current_user_added_count( $product_id ) : 0;

		// if no user added to wishlist, return empty string.
		if ( ! $count ) {
			return apply_filters( 'mwb_wcwl_count_text_empty', '', $product_id );
		} elseif ( ! $current_user_count ) {
			// translators: 1. Number of users.
			$count_text = sprintf( _n( '%d user', '%d users', $count, 'mwb-woocommerce-wishlist' ), $count );
			$text       = _n( 'has this item in wishlist', 'have this item in wishlist', $count, 'mwb-woocommerce-wishlist' );
		} elseif ( $count == $current_user_count ) {
			$count_text = __( 'You\'re the first', 'mwb-woocommerce-wishlist' );
			$text       = __( 'to add this item in wishlist', 'mwb-woocommerce-wishlist' );
		} else {
			$other_count = $count - $current_user_count;
			// translators: 1. Count of users when many, or "another" when only one.
			$count_text = sprintf( _n( 'You and %s user', 'You and %d users', $other_count, 'mwb-woocommerce-wishlist' ), 1 == $other_count ? __( 'another', 'mwb-woocommerce-wishlist' ) : $other_count );
			$text       = __( 'have this item in wishlist', 'mwb-woocommerce-wishlist' );
		}

		$label = sprintf( '<div class="count-add-to-wishlist"><span class="count">%s</span> %s</div>', $count_text, $text );

		return apply_filters( 'mwb_wcwl_count_text', $label, $product_id, $current_user_count, $count );
	}
}

/* === COOKIE FUNCTIONS === */

if ( ! function_exists( 'mwb_wcwl_get_cookie_expiration' ) ) {
	/**
	 * Returns default expiration for wishlist cookie
	 *
	 * @return int Number of seconds the cookie should last.
	 */
	function mwb_wcwl_get_cookie_expiration() {
		return intval( apply_filters( 'mwb_wcwl_cookie_expiration', 60 * 60 * 24 * 30 ) );
	}
}

if ( ! function_exists( 'mwb_setcookie' ) ) {
	/**
	 * Create a cookie.
	 *
	 ...
	 * @since 1.0.0
	 */
	function mwb_setcookie( $name, $value = array(), $time = null, $secure = false, $httponly = false ) {
		if ( ! apply_filters( 'mwb_wcwl_set_cookie', true ) || empty( $name ) ) {
			return false;
		}

		$time = null != $time ? $time : time() + mwb_wcwl_get_cookie_expiration();

		$value      = json_encode( stripslashes_deep( $value ) );
		$expiration = apply_filters( 'mwb_wcwl_cookie_expiration_time', $time ); // Default 30 days.
...
if ( ! function_exists( 'mwb_getcookie' ) ) {
	/**
...
if ( ! function_exists( 'mwb_destroycookie' ) ) {
	/**
	 * Destroy a cookie.
	 *
	 * @param string $name Cookie name.
	 ...
	 * @since 1.0.0
	 */
	function mwb_destroycookie( $name ) {
		mwb_setcookie( $name, array(), time() - 3600 );
	}
}

/* === GET FUNCTIONS === */

if ( ! function_exists( 'mwb_wcwl_get_hidden_products' ) ) {
	/**
	 ...
	 * @since 2.1.1
	 */
	function mwb_wcwl_get_hidden_products() {
		 ...
		 * @since 2.2.6
		 */
		return apply_filters( 'mwb_wcwl_hidden_products', array_filter( $hidden_products ) );
	}
}

if ( ! function_exists( 'mwb_wcwl_get_wishlist' ) ) {
	/**
	 * Retrieves wishlist by ID
	 *
	 * @param int|string $wishlist_id Wishlist ID or Wishlist Token.
	 *
	 * @return \mwb_WCWL_Wishlist|bool Wishlist object; false on error
	 */
	function mwb_wcwl_get_wishlist( $wishlist_id ) {
		return mwb_WCWL_Wishlist_Factory::get_wishlist( $wishlist_id );
	}
}

if ( ! function_exists( 'mwb_wcwl_get_plugin_icons' ) ) {
	/**
	 ...
	 */
	function mwb_wcwl_get_plugin_icons( $none_label = '', $custom_label = '' ) {
		$icons = json_decode( file_get_contents( mwb_WCWL_DIR . 'assets/js/admin/mwb-wcwl-icons.json' ), true );

		$icons['none']   = $none_label ? $none_label : __( 'None', 'mwb-woocommerce-wishlist' );
		$icons['custom'] = $custom_label ? $custom_label : __( 'Custom', 'mwb-woocommerce-wishlist' );

		return $icons;
	}
}

if ( ! function_exists( 'mwb_wcwl_get_privacy_label' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	function mwb_wcwl_get_privacy_label( $privacy, $extended = false ) {

		switch ( $privacy ) {
			case 1:
				$privacy_label = 'shared';
				$privacy_text  = __( 'Shared', 'mwb-woocommerce-wishlist' );

				if ( $extended ) {
					$privacy_text = '<b>' . $privacy_text . '</b> - ';
					$privacy_text .= __( 'Only people with a link to this list can see it', 'mwb-woocommerce-wishlist' );
				}

				break;
			case 2:
				$privacy_label = 'private';
				$privacy_text  = __( 'Private', 'mwb-woocommerce-wishlist' );

				if ( $extended ) {
					$privacy_text = '<b>' . $privacy_text . '</b> - ';
					$privacy_text .= __( 'Only you can see this list', 'mwb-woocommerce-wishlist' );
				}

				break;
			default:
				$privacy_label = 'public';
				$privacy_text  = __( 'Public', 'mwb-woocommerce-wishlist' );

				if ( $extended ) {
					$privacy_text = '<b>' . $privacy_text . '</b> - ';
					$privacy_text .= __( 'Anyone can search for and see this list', 'mwb-woocommerce-wishlist' );
				}

				break;
		}

		return apply_filters( "mwb_wcwl_{$privacy_label}_wishlist_visibility", $privacy_text, $extended, $privacy );
	}
}

if ( ! function_exists( 'mwb_wcwl_get_privacy_value' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	function mwb_wcwl_get_privacy_value( $privacy_label ) {
		...
		return apply_filters( 'mwb_wcwl_privacy_value', $privacy_value, $privacy_label );
	}
}

if ( ! function_exists( 'mwb_wcwl_get_current_url' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	function mwb_wcwl_get_current_url() {
		global $wp;
		 ...
		 * @since 3.0.12
		 */
		return apply_filters( 'mwb_wcwl_current_url', '', add_query_arg( $wp->query_vars, home_url( $wp->request ) ) );
	}
}

/* === UTILITY FUNCTIONS === */

if ( ! function_exists( 'mwb_wcwl_merge_in_array' ) ) {
	/**
	 ...
	 */
	function mwb_wcwl_merge_in_array( $array, $element, $pivot, $position = 'after' ) {
...
if ( ! function_exists( 'mwb_wcwl_maybe_format_field_array' ) ) {
	/**
	 ...
	 */
	function mwb_wcwl_maybe_format_field_array( $field_structure ) {
		$fields = array();
...
if ( ! function_exists( 'mwb_wcwl_add_notice' ) ) {
	/**
	 ...
	 * @since 3.0.10
	 */
	function mwb_wcwl_add_notice( $message, $notice_type = 'success', $data = array() ) {
		function_exists( 'wc_add_notice' ) && wc_add_notice( $message, $notice_type, $data );
	}
}

if ( ! function_exists( 'mwb_wcwl_object_id' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 */
	function mwb_wcwl_object_id( $id, $type = 'page', $return_original = true, $lang = null ) {
		...
		$id = apply_filters( 'mwb_wcwl_object_id', $id, $type, $return_original, $lang );
...
if ( ! function_exists( 'mwb_frontend_css_color_picker' ) ) {
	/**
	 * Output a colour picker input box.
	 *
	 * This function is not of the plugin mwb WCWL. It is from WooCommerce.
	 ...
	 * @deprecated
	 */
	function mwb_frontend_css_color_picker( $name, $id, $value, $desc = '' ) {
		_deprecated_function( 'mwb_frontend_css_color_picker', '3.0.0' );
