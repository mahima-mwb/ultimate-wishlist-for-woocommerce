...
if ( ! class_exists( 'mwb_WCWL_Wishlist' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	class mwb_WCWL_Wishlist extends WC_Data implements ArrayAccess {
		 ...
		 * Constructor
		 *
		 * @param $wishlist int|string|\mwb_WCWL_Wishlist Wishlist identifier
		...
		public function __construct( $wishlist = 0 ) {
			// set default values
			$this->data = array(
				'privacy'            => apply_filters( 'mwb_wcwl_default_wishlist_privacy', 0 ),
				'user_id'            => 0,
				'session_id'         => '',
				'name'               => apply_filters( 'mwb_wcwl_default_wishlist_name', '' ),
				'slug'               => apply_filters( 'mwb_wcwl_default_wishlist_slug', '' ),
				'token'              => '',
				'is_default'         => 0,
				'date_added'         => '',
				'expiration'         => ''
		...
		public function is_current_user_owner( $current_user = false ) {
			...
			if( $this->is_session_based() && mwb_WCWL_Session()->get_session_id() == $session_id ){
				return true;
			}
		...
		public function current_user_can( $capability = 'view', $current_user = false ) {
			...
			if( is_user_logged_in() && current_user_can( 'manage_woocommerce' ) && apply_filters( 'mwb_wcwl_admin_can', true, $capability, $current_user, $this ) ){
				return true;
			}
			...
			return apply_filters( 'mwb_wcwl_current_user_can', $can, $capability, $current_user, $this );
		}
		...
		public function get_formatted_privacy( $context = 'view' ) {
			...
			$formatted_privacy = mwb_wcwl_get_privacy_label( $privacy );

			return apply_filters( 'mwb_wcwl_wishlist_formatted_privacy', $formatted_privacy, $privacy, $this, $context );
		}
		...
		public function has_privacy ( $privacy ) {
			...
			elseif( is_string( $privacy ) ){
				$has_privacy = mwb_wcwl_get_privacy_value( $privacy ) == $wishlist_privacy;
			}
			else{
		...
		public function get_formatted_name( $context = 'view' ) {
			...
			if( $this->is_default() && ! $name ){
				$name = apply_filters( 'mwb_wcwl_default_wishlist_formatted_title', get_option( 'mwb_wcwl_wishlist_title' ) );
			}

			return apply_filters( 'mwb_wcwl_wishlist_formatted_title', $name );
		}
		...
		public function get_url() {
			return mwb_WCWL()->get_wishlist_url( 'view/' . $this->get_token() );
		}
		...
		public function get_download_url() {
			return apply_filters( 'mwb_wcwl_wishlist_download_url', wp_nonce_url( add_query_arg( 'download_wishlist', $this->get_id() ), 'download_wishlist', 'download_nonce' ), $this );
		}
		...
		public function get_delete_url() {
			return apply_filters( 'mwb_wcwl_wishlist_delete_url', wp_nonce_url( add_query_arg( 'wishlist_id', $this->get_id(), mwb_WCWL()->get_wishlist_url( 'manage' ) ), 'mwb_wcwl_delete_action', 'mwb_wcwl_delete' ), $this );
		}
		...
		public function get_items( $limit = 0, $offset = 0 ) {
			...
			$items = apply_filters( 'mwb_wcwl_wishlist_get_items', $this->items, $this );
		 ...
		 * @return \mwb_WCWL_Wishlist_Item|bool Item on success, false on failure
		 */
		 ...
		 *
		 * @param $item_id int Item id
		 * @return \mwb_WCWL_Wishlist_Item|bool Item to retrieve, or false on error
		 */
		 ...
		 *
		 * @param $item \mwb_WCWL_Wishlist_Item
		 * @return \mwb_WCWL_Wishlist_Item|bool Item on success; false on failure
		 */
		...
		protected function map_legacy_offsets( $offset ) {
			...
			return apply_filters( 'mwb_wcwl_wishlist_map_legacy_offsets', $offset, $legacy_offset );
		}
	}
}
