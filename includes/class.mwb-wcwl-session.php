...
if ( ! class_exists( 'mwb_WCWL_Session' ) ) {
	/**
	 ...
	 * @since 3.0.0
	 */
	class mwb_WCWL_Session {
		/**
		 * Single instance of the class
		 *
		 * @var \mwb_WCWL_Session
		 * @since 3.0.0
		 */
		...
		public function set_session_cookie() {
			if ( headers_sent() || ! apply_filters( 'mwb_wcwl_set_session_cookie', true ) ) {
				return;
			}
			...
			mwb_setcookie( $this->get_session_cookie_name(), $cookie_value, $this->_session_expiration, $this->use_secure_cookie(), true );
		...
		public function get_session_cookie() {
			$cookie_value = mwb_getcookie( $this->get_session_cookie_name() ); // @codingStandardsIgnoreLine.
		...
		public function use_secure_cookie() {
			return apply_filters( 'mwb_wcwl_session_use_secure_cookie', wc_site_is_https() && is_ssl() );
		}
		...
		public function get_session_cookie_name() {
			if ( empty( $this->_cookie ) ) {
				$this->_cookie = apply_filters( 'mwb_wcwl_session_cookie', 'mwb_wcwl_session_' . COOKIEHASH );
			}
		...
		public function set_session_expiration() {
			$this->_session_expiring   = time() + mwb_wcwl_get_cookie_expiration() - HOUR_IN_SECONDS;
			$this->_session_expiration = time() + mwb_wcwl_get_cookie_expiration();
		}
		...
		public function forget_session() {
			mwb_destroycookie( $this->get_session_cookie_name() );
...
/**
 * Unique access to instance of mwb_WCWL_Session class
 *
 * @return \mwb_WCWL_Session
 * @since 3.0.0
 */
function mwb_WCWL_Session() {
	return mwb_WCWL_Session::get_instance();
}
