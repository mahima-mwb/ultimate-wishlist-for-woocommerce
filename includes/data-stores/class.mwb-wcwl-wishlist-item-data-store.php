...
if ( ! class_exists( 'mwb_WCWL_Wishlist_Item_Data_Store' ) ) {
	/**
	 * This class implements CRUD methods for wishlists' items
	 *
	 * @since 3.0.0
	 */
	class mwb_WCWL_Wishlist_Item_Data_Store{

		/**
		 * Create a new wishlist item in the database.
		 *
		 * @since 3.0.0
		 * @param \mwb_WCWL_Wishlist_Item $item Wishlist item object.
		 */
		public function create( &$item ) {
			...
			if( $item_id = mwb_WCWL_Wishlist_Factory::get_wishlist_item_by_product_id( $wishlist_id, $product_id ) ){
				$item->set_id( $item_id );
			...
			$values = array(
				apply_filters( 'mwb_wcwl_adding_to_wishlist_product_id', $product_id ),
				apply_filters( 'mwb_wcwl_adding_to_wishlist_quantity', $item->get_quantity() ),
				apply_filters( 'mwb_wcwl_adding_to_wishlist_wishlist_id', $wishlist_id ),
				apply_filters( 'mwb_wcwl_adding_to_wishlist_position', $item->get_position() ),
				apply_filters( 'mwb_wcwl_adding_to_wishlist_original_price', $item->get_product_price() ),
				apply_filters( 'mwb_wcwl_adding_to_wishlist_original_currency', $item->get_original_currency() ),
				apply_filters( 'mwb_wcwl_adding_to_wishlist_on_sale', $item->is_on_sale() ),
			);

			if ( $user_id = $item->get_user_id() ) {
				$columns['user_id'] = '%d';
				$values[] = apply_filters( 'mwb_wcwl_adding_to_wishlist_user_id', $user_id );
			}

			if( $date_added = $item->get_date_added( 'edit' ) ){
				$columns['dateadded'] = 'FROM_UNIXTIME( %d )';
				$values[] = apply_filters( 'mwb_wcwl_adding_to_wishlist_date_added', $date_added->getTimestamp() );
			}

			$query_columns = implode( ', ', array_map( 'esc_sql', array_keys( $columns ) ) );
			$query_values = implode( ', ', array_values( $columns ) );
			$query = "INSERT INTO {$wpdb->mwb_wcwl_items} ( {$query_columns} ) VALUES ( {$query_values} ) ";
			...
			if( $res ) {
				...
				do_action( 'mwb_wcwl_new_wishlist_item', $item->get_id(), $item, $item->get_wishlist_id() );
			}
		}
		...
		public function read( &$item ) {
			...
			if ( false === $data ) {
				$data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->mwb_wcwl_items} WHERE ID = %d LIMIT 1;", $item->get_id() ) );
				wp_cache_set( 'item-' . $item->get_id(), $data, 'wishlist-items' );
			}

			if ( ! $data ) {
				throw new Exception( __( 'Invalid wishlist item.', 'mwb-woocommerce-wishlist' ) );
			}
		...
		public function update( &$item ) {
			...
			do_action( 'mwb_wcwl_update_wishlist_item', $item->get_id(), $item, $item->get_wishlist_id() );
		}
		...
		public function delete( &$item ) {
			...
			do_action( 'mwb_wcwl_before_delete_wishlist_item', $item->get_id() );

			$wpdb->delete( $wpdb->mwb_wcwl_items, array( 'ID' => $item->get_id() ) );

			do_action( 'mwb_wcwl_delete_wishlist_item', $item->get_id() );
		...
		public function query( $args = array() ) {
			...
			$sql = "SELECT SQL_CALC_FOUND_ROWS i.*
                    ...
                    LEFT JOIN {$wpdb->mwb_wcwl_wishlists} AS l ON l.`ID` = i.`wishlist_id`
			...
			if( ! empty( $product_id ) ){
				$product_id = mwb_wcwl_object_id( $product_id, 'product', true, 'default' );
			...
			if( ! empty( $is_default ) ){
				mwb_WCWL_Wishlist_Factory::generate_default_wishlist();
			...
			if( ! empty( $items ) ){
				$items = array_map( array( 'mwb_WCWL_Wishlist_Factory', 'get_wishlist_item' ), $items );
			} else {
				$items = array();
			}

			return apply_filters( 'mwb_wcwl_get_products', $items, $args );
		}
			...
			if( $user ){
				if( 'current' == $user ){
					if( is_user_logged_in() ){
						...
						$query_args[] = mwb_WCWL_Session()->get_session_id();
					}
				}
				elseif( is_int( $user ) ){
					$user_condition = " AND l.`user_id` = %d";
			...
			$query = "SELECT 
					  ...
					  FROM ( 
					      SELECT 
					          ...
					          l.`ID` as wishlist_id 
					      FROM {$wpdb->mwb_wcwl_wishlists} AS l 
					      WHERE ( l.`expiration` > NOW() OR l.`expiration` IS NULL ) {$user_condition}
				      ) as v 
				      LEFT JOIN {$wpdb->mwb_wcwl_items} AS i USING( wishlist_id ) 
					  WHERE i.`prod_id` = %d";
		...
		public function update_raw( $columns, $column_values, $conditions = array(), $conditions_values = array(), $clear_caches = false ) {
			...
			if( $clear_caches ){
				$query = "SELECT ID FROM {$wpdb->mwb_wcwl_items} {$query_where}";
			...
			$query = "UPDATE {$wpdb->mwb_wcwl_items} SET {$query_columns} {$query_where}";
		 ...
		 * Clear meta cache.
		 *
		 * @param mwb_WCWL_Wishlist_Item|int $item Wishlist item object, or id of the item.
		 */
		public function clear_cache( &$item ) {
			if( ! $item instanceof mwb_WCWL_Wishlist_Item ){
				$item = mwb_WCWL_Wishlist_Factory::get_wishlist_item( $item );
			}
		...
		public static function filter_join_for_wishlist_count( $join ) {
			global $wpdb;
			$join .= " LEFT JOIN ( SELECT COUNT(*) AS wishlist_count, prod_id FROM {$wpdb->mwb_wcwl_items} GROUP BY prod_id ) AS i ON ID = i.prod_id";
			return $join;
		}
