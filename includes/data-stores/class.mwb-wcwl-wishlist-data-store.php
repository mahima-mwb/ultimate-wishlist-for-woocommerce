...
if ( ! class_exists( 'mwb_WCWL_Wishlist_Data_Store' ) ) {
	/**
	 * This class implements CRUD methods for wishlists
	 *
	 * @since 3.0.0
	 */
	class mwb_WCWL_Wishlist_Data_Store{

		/**
		 * Create a new wishlist and stores it on DB
		 *
		 * @param $wishlist \mwb_WCWL_Wishlist Wishlist to create
		 */
		public function create( &$wishlist ){
			...
			if( ! $session_id = $wishlist->get_session_id() && ! $user_id = $wishlist->get_user_id() ){
				if( is_user_logged_in() ){
					...
					$wishlist->set_user_id( apply_filters( 'mwb_wcwl_add_wishlist_user_id', $user_id ) );
				}
				else{
					$session_id = mwb_WCWL_Session()->get_session_id();
					$wishlist->set_session_id( apply_filters( 'mwb_wcwl_add_wishlist_session_id', $session_id ) );
				}
			}
			...
			$values = array(
				apply_filters( 'mwb_wcwl_add_wishlist_privacy', $wishlist->get_privacy() ),
				apply_filters( 'mwb_wcwl_add_wishlist_name', $wishlist->get_name() ),
				apply_filters( 'mwb_wcwl_add_wishlist_slug', $wishlist->get_slug() ),
				apply_filters( 'mwb_wcwl_add_wishlist_token', $wishlist->get_token() ),
				apply_filters( 'mwb_wcwl_add_wishlist_is_default', $wishlist->get_is_default() )
			);

			if( $session_id = $wishlist->get_session_id() ){
				$columns['session_id'] = '%s';
				$values[] = apply_filters( 'mwb_wcwl_add_wishlist_session_id', $session_id );
			}

			if( $user_id = $wishlist->get_user_id() ){
				$columns['user_id'] = '%d';
				$values[] = apply_filters( 'mwb_wcwl_add_wishlist_user_id', $user_id );
			}

			if( $date_added = $wishlist->get_date_added( 'edit' ) ){
				$columns['dateadded'] = 'FROM_UNIXTIME( %d )';
				$values[] = apply_filters( 'mwb_wcwl_add_wishlist_date_added', $date_added->getTimestamp() );
			}

			if( $expiration = $wishlist->get_expiration( 'edit' ) ){
				$columns['expiration'] = 'FROM_UNIXTIME( %d )';
				$values[] = apply_filters( 'mwb_wcwl_add_wishlist_expiration', $expiration->getTimestamp() );
			}

			// if session wishlist, set always an expiration
			if( isset( $columns['session_id'] ) && ! $expiration && $expiration = mwb_WCWL_Session()->get_session_expiration() ){
				$columns['expiration'] = 'FROM_UNIXTIME( %d )';
				$values[] = apply_filters( 'mwb_wcwl_add_wishlist_expiration', $expiration );
			}

			$query_columns = implode( ', ', array_map( 'esc_sql', array_keys( $columns ) ) );
			$query_values = implode( ', ', array_values( $columns ) );
			$query = "INSERT INTO {$wpdb->mwb_wcwl_wishlists} ( {$query_columns} ) VALUES ( {$query_values} ) ";

			$res = $wpdb->query( $wpdb->prepare( $query, $values ) );

			if ( $res ) {
				$id = apply_filters( 'mwb_wcwl_wishlist_correctly_created', intval( $wpdb->insert_id ) );

				$wishlist->set_id( $id );
				$wishlist->apply_changes();
				$this->clear_caches( $wishlist );

				do_action( 'mwb_wcwl_new_wishlist', $wishlist->get_id(), $wishlist );
			}
		}
		 ...
		 *
		 * @param $wishlist \mwb_WCWL_Wishlist
		...
		public function read( &$wishlist ){
			...
			if ( ! ( $id = $wishlist->get_id() ) && ! ( $token = $wishlist->get_token() ) ) {
				throw new Exception( __( 'Invalid wishlist.', 'mwb-woocommerce-wishlist' ) );
			}
			...
			if( ! $wishlist_data ) {
				...
				$query = false;
				if ( $id ) {
					$query = $wpdb->prepare( "SELECT * FROM {$wpdb->mwb_wcwl_wishlists} WHERE ID = %d", $id );
				} elseif ( $token ) {
					$query = $wpdb->prepare( "SELECT * FROM {$wpdb->mwb_wcwl_wishlists} WHERE wishlist_token = %s", $token );
				}
		 ...
		 *
		 * @param $wishlist \mwb_WCWL_Wishlist Wishlist to save on db, with $changes property
		 */
		public function update( &$wishlist ){
			...
			if ( array_intersect( array( 'user_id', 'session_id', 'slug', 'name', 'token', 'privacy', 'expiration', 'date_added', 'is_default' ), array_keys( $changes ) ) ) {
				...
				if( $session_id = $wishlist->get_session_id() ){
					$columns['session_id'] = '%s';
					$values[] = apply_filters( 'mwb_wcwl_update_wishlist_session_id', $session_id );
				}
				else{
				...
				if( $user_id = $wishlist->get_user_id() ){
					$columns['user_id'] = '%d';
					$values[] = apply_filters( 'mwb_wcwl_update_wishlist_user_id', $user_id );
				}
				else{
				...
				if( $expiration = $wishlist->get_expiration( 'edit' ) ){
					$columns['expiration'] = 'FROM_UNIXTIME( %d )';
					$values[] = apply_filters( 'mwb_wcwl_update_wishlist_expiration', $expiration->getTimestamp() );
				}
				else{
			...
			do_action( 'mwb_wcwl_update_wishlist', $wishlist->get_id(), $wishlist );
		}

		/**
		 * Delete a wishlist from DB
		 *
		 * @param \mwb_WCWL_Wishlist $wishlist Wishlist to delete.
		 */
		public function delete( &$wishlist ) {
			...
			do_action( 'mwb_wcwl_before_delete_wishlist', $wishlist->get_id() );

			$this->clear_caches( $wishlist );

			// delete wishlist and all its items.
			$wpdb->delete( $wpdb->mwb_wcwl_items, array( 'wishlist_id' => $id ) );
			$wpdb->delete( $wpdb->mwb_wcwl_wishlists, array( 'ID' => $id ) );

			do_action( 'mwb_wcwl_delete_wishlist', $wishlist->get_id() );

			$wishlist->set_id( 0 );

			do_action( 'mwb_wcwl_deleted_wishlist', $id );
		...
		public function delete_expired() {
			...
			$wpdb->query( "DELETE FROM {$wpdb->mwb_wcwl_items} WHERE wishlist_id IN ( SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} WHERE expiration < NOW() and user_id IS NULL )" );
			$wpdb->query( "DELETE FROM {$wpdb->mwb_wcwl_wishlists} WHERE expiration < NOW() and user_id IS NULL" );
		}

		/**
		 * Query database to search
		 *
		 * @return \mwb_WCWL_Wishlist[] Array of matched wishlists.
		 */
		public function query( $args = array() ) {
			...
			$default = array(
				...
				'session_id' => ( ! is_user_logged_in() ) ? mwb_WCWL_Session()->get_session_id() : false,
				'wishlist_slug' => false,
				'wishlist_name' => false,
				'wishlist_token' => false,
				'wishlist_visibility' => apply_filters( 'mwb_wcwl_wishlist_visibility_string_value', 'all'), // all, visible, public, shared, private
				'user_search' => false,
				's' => false,
				'is_default' => false,
				'orderby' => '',
			...
			$sql .= " FROM `{$wpdb->mwb_wcwl_wishlists}` AS l";
			...
			if( ! empty( $wishlist_visibility ) && $wishlist_visibility != 'all' ){
				switch( $wishlist_visibility ){
					...
					default:
						...
						$sql_args[] = mwb_wcwl_get_privacy_value( $wishlist_visibility );
						break;
				}
			}

			if( empty( $show_empty ) ){
				$sql .= " AND l.`ID` IN ( SELECT wishlist_id FROM {$wpdb->mwb_wcwl_items} )";
			}
			...
			if( ! empty( $lists ) ){
				$lists = array_map( array( 'mwb_WCWL_Wishlist_Factory', 'get_wishlist' ), $lists );
			} else {
				$lists = array();
			}

			return apply_filters( 'mwb_wcwl_get_wishlists', $lists, $args );
		}
		...
		public function search_users( $args = array() ) {
			...
			$sql = "SELECT DISTINCT i.user_id
                    FROM {$wpdb->mwb_wcwl_items} AS i
                    LEFT JOIN {$wpdb->mwb_wcwl_wishlists} AS l ON i.wishlist_id = l.ID";
		...
		public function update_raw( $columns, $column_values, $conditions = array(), $conditions_values = array(), $clear_caches = false ) {
			...
			if( $clear_caches ){
				$query = "SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} {$query_where}";
			...
			$query = "UPDATE {$wpdb->mwb_wcwl_wishlists} SET {$query_columns} {$query_where}";
		 ...
		 *
		 * @param $wishlist \mwb_WCWL_Wishlist
		 * @return \mwb_WCWL_Wishlist_Item[] Array or Wishlist items for the wishlist
		 */
		public function read_items( $wishlist ) {
			...
			if ( false === $items ) {
				$query = "SELECT i.* FROM {$wpdb->mwb_wcwl_items} as i INNER JOIN {$wpdb->posts} as p on i.prod_id = p.ID WHERE wishlist_id = %d AND p.post_type IN ( %s, %s ) AND p.post_status = %s";

				// remove hidden products from result
				$hidden_products = mwb_wcwl_get_hidden_products();

				if( ! empty( $hidden_products ) && apply_filters( 'mwb_wcwl_remove_hidden_products_via_query', true ) ){
				 ...
				 * @since 3.0.7
				 */
				if( ! empty( $hidden_products ) && ! empty( $items ) && ! apply_filters( 'mwb_wcwl_remove_hidden_products_via_query', true ) ){
			...
			if ( ! empty( $items ) ) {
				$items = array_map( array( 'mwb_WCWL_Wishlist_Factory', 'get_wishlist_item' ), array_combine( wp_list_pluck( $items, 'prod_id' ), $items ) );
			} else {
				$items = array();
			}

			return apply_filters( 'mwb_wcwl_get_products', $items, array( 'wishlist_id' => $wishlist->get_id() ) );
		}
		...
		public function delete_items( $wishlist ) {
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->mwb_wcwl_items} WHERE wishlist_id = %d", $wishlist->get_id() ) );
		...
		public function generate_token() {
			...
			$sql = "SELECT COUNT(*) FROM `{$wpdb->mwb_wcwl_wishlists}` WHERE `wishlist_token` = %s";
		...
		public function assign_to_user( $session_id, $user_id ) {
			...
			$items = $wpdb->get_col( $wpdb->prepare( "SELECT i.ID FROM {$wpdb->mwb_wcwl_items} AS i LEFT JOIN {$wpdb->mwb_wcwl_wishlists} AS l ON l.ID = i.wishlist_id WHERE l.session_id = %s", $session_id ) );

			if( ! empty( $items ) ) {
				$items_string = implode( ',', array_map( 'esc_sql', $items ) );
				$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->mwb_wcwl_items} SET user_id = %d WHERE ID IN ({$items_string})", $user_id ) );
			}
			...
			$default_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} WHERE is_default = %d AND user_id = %d ORDER BY dateadded ASC", 1, $user_id ) );
			...
			if( count( $default_ids ) > 1 ){
				...
				try{
					...
					if( apply_filters( 'mwb_wcwl_merge_default_wishlists', true ) ){
						// change wishlist id to master default id
						$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->mwb_wcwl_items} SET wishlist_id = %d WHERE wishlist_id IN ({$where_statement})", $master_default_wishlist ) );

						// delete slave default wishlists
						$wpdb->query( "DELETE FROM {$wpdb->mwb_wcwl_wishlists} WHERE ID IN ({$where_statement})" );
					}

					// otherwise, we just leave all the wishlists as they are, but we remove default flag from latest
					else {
						// remove default flag
						$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->mwb_wcwl_wishlists} SET is_default = %d WHERE ID IN ({$where_statement})", 0 ) );

						// set name where it is missing
						$default_title = apply_filters( 'mwb_wcwl_default_wishlist_formatted_title', get_option( 'mwb_wcwl_wishlist_title' ) );
						$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->mwb_wcwl_wishlists} SET wishlist_name = %s WHERE ID IN ({$where_statement}) AND wishlist_name = ''", $default_title ) );
					}
				}
				catch( Exception $e ){
					return;
		 ...
		 * @return \mwb_WCWL_Wishlist|bool Default wishlist for current user/session, or false on failure
		 */
		public function get_default_wishlist( $id = false, $context = 'read' ) {
			...
			if( ! empty( $id ) && is_int( $id ) ){
				...
				$wishlist_id = $wishlist_id !== false ? $wishlist_id : $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} WHERE user_id = %d AND is_default = 1", $id ) );
			}
			elseif( ! empty( $id ) && is_string( $id ) ){
				$cache_key = 'wishlist-default-' . $id;
				$wishlist_id = wp_cache_get( $cache_key, 'wishlists' );
				$wishlist_id = $wishlist_id !== false ? $wishlist_id : $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} WHERE session_id = %s AND expiration > NOW() AND is_default = 1", $id ) );
			}
			elseif( $user_id = get_current_user_id() ){
				$cache_key = 'wishlist-default-' . $user_id;
				$wishlist_id = wp_cache_get( $cache_key, 'wishlists' );
				$wishlist_id = $wishlist_id !== false ? $wishlist_id : $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} WHERE user_id = %d AND is_default = 1", $user_id ) );
			}
			elseif( $session_id = mwb_WCWL_Session()->get_session_id() ){
				$cache_key = 'wishlist-default-' . $session_id;
				$wishlist_id = wp_cache_get( $cache_key, 'wishlists' );
				$wishlist_id = $wishlist_id !== false ? $wishlist_id : $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->mwb_wcwl_wishlists} WHERE session_id = %s AND expiration > NOW() AND is_default = 1", $session_id ) );
			}
			...
			if( $wishlist_id ){
				...
				return mwb_WCWL_Wishlist_Factory::get_wishlist( $wishlist_id );
			}
			elseif( 'edit' == $context ){
		 ...
		 * @return mwb_WCWL_Wishlist|bool Brand new default wishlist, or false on failure
		 */
		public function generate_default_wishlist( $id ) {
			try {
				$default_wishlist = new mwb_WCWL_Wishlist();
				 ...
				 * @since 3.0.10
				 */
				do_action( 'mwb_wcwl_generated_default_wishlist', $default_wishlist, $id );
			}
			catch( Exception $e ){
				return false;
			}
		...
		public function slug_exists( $slug ) {
			...
			$res = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(ID) FROM {$wpdb->mwb_wcwl_wishlists} WHERE wishlist_slug = %s", $slug ) );
		...
		protected function should_be_default() {
			...
			if( $user_id = get_current_user_id() ){
				$wishlists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( ID ) FROM {$wpdb->mwb_wcwl_wishlists} WHERE user_id = %d AND is_default = %d", $user_id, 1 ) );

				return ! (bool) $wishlists;
			}

			if( $customer_id = mwb_WCWL_Session()->get_session_id() ){
				$wishlists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( ID ) FROM {$wpdb->mwb_wcwl_wishlists} WHERE session_id = %s AND expiration > NOW() AND is_default = %d", $customer_id, 1 ) );
		 ...
		 *
		 * @param $wishlist \mwb_WCWL_Wishlist|int|string
		 * @return void
		 */
		protected function clear_caches( &$wishlist ) {
			if( $wishlist instanceof mwb_WCWL_Wishlist ){
				$id = $wishlist->get_id();
				$token = $wishlist->get_token();
			}
			elseif( intval( $wishlist ) ){
				$id = $wishlist;
				$wishlist = mwb_wcwl_get_wishlist( $wishlist );
