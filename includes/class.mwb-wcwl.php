...
if ( ! class_exists( 'mwb_WCWL' ) ) {
	...
	class mwb_WCWL {
		...
		public function __construct() {
			...
			// init frontend class
			$this->wcwl_frontend = mwb_WCWL_Frontend();

			// init crons
			$this->wcwl_cron = mwb_WCWL_Cron();

			// init session
			$this->wcwl_session = mwb_WCWL_Session();

			// init admin handling
			if( is_admin() ){
				$this->wcwl_admin = mwb_WCWL_Admin();
			}
		...
		public function privacy_loader() {
			if( class_exists( 'mwb_Privacy_Plugin_Abstract' ) ) {
				require_once( mwb_WCWL_INC . 'class.mwbh-wcwl-privacy.php' );
				new mwb_WCWL_Privacy();
			}
		}
		 ...
		 * @throws mwb_WCWL_Exception When an error occurs with Add to Wishlist operation
		 * @since 1.0.0
		 */
		public function add( $atts = array() ) {
			...
			// filtering params
			$prod_id     = apply_filters( 'mwb_wcwl_adding_to_wishlist_prod_id', intval( $atts['add_to_wishlist'] ) );
			$wishlist_id = apply_filters( 'mwb_wcwl_adding_to_wishlist_wishlist_id', $atts['wishlist_id'] );
			$quantity    = apply_filters( 'mwb_wcwl_adding_to_wishlist_quantity', intval( $atts['quantity'] ) );
			$user_id     = apply_filters( 'mwb_wcwl_adding_to_wishlist_user_id', intval( $atts['user_id'] ) );
			$dateadded   = apply_filters( 'mwb_wcwl_adding_to_wishlist_dateadded', $atts['dateadded'] );

			do_action( 'mwb_wcwl_adding_to_wishlist', $prod_id, $wishlist_id, $user_id );

			if( ! $this->can_user_add_to_wishlist() ){
				throw new mwb_WCWL_Exception( apply_filters( 'mwb_wcwl_user_cannot_add_to_wishlist_message', __( 'The item cannot be added to this wishlist', 'mwb-woocommerce-wishlist' ) ), 1 );
			}

			if ( $prod_id == false ) {
				throw new mwb_WCWL_Exception( __( 'An error occurred while adding the products to the wishlist.', 'mwb-woocommerce-wishlist' ), 0 );
			}

			$wishlist = 'new' === $wishlist_id ? $this->add_wishlist( $atts ) : mwb_WCWL_Wishlist_Factory::get_wishlist( $wishlist_id, 'edit' );

			if( ! $wishlist instanceof mwb_WCWL_Wishlist || ! $wishlist->current_user_can( 'add_to_wishlist' ) ){
				throw new mwb_WCWL_Exception( __( 'An error occurred while adding the products to the wishlist.', 'mwb-woocommerce-wishlist' ), 0 );
			}
			...
			if( $wishlist->has_product( $prod_id ) ) {
				throw new mwb_WCWL_Exception( apply_filters( 'mwb_wcwl_product_already_in_wishlist_message', get_option( 'mwb_wcwl_already_in_wishlist_text' ) ), 1 );
			}

			$item = new mwb_WCWL_Wishlist_Item();
			...
			do_action( 'mwb_wcwl_added_to_wishlist', $prod_id, $item->get_wishlist_id(), $item->get_user_id() );
		}
		...
		public function remove( $atts = array() ) {
			...
			do_action( 'mwb_wcwl_removing_from_wishlist', $prod_id, $wishlist_id, $user_id );

			if( $prod_id == false ){
				throw new mwb_WCWL_Exception( apply_filters( 'mwb_wcwl_unable_to_remove_product_message', __( 'Error. Unable to remove the product from the wishlist.', 'mwb-woocommerce-wishlist' ) ), 0 );
			}

			$wishlist = mwb_WCWL_Wishlist_Factory::get_wishlist( $wishlist_id );

			if( ! $wishlist instanceof mwb_WCWL_Wishlist || ! $wishlist->current_user_can( 'remove_from_wishlist' ) ){
				throw new mwb_WCWL_Exception( apply_filters( 'mwb_wcwl_unable_to_remove_product_message', __( 'Error. Unable to remove the product from the wishlist.', 'mwb-woocommerce-wishlist' ) ), 0 );
			}
			...
			do_action( 'mwb_wcwl_removed_from_wishlist', $prod_id, $wishlist->get_id(), $wishlist->get_user_id() );
		}
		...
		public function is_product_in_wishlist( $product_id, $wishlist_id = false ) {
			...
			return apply_filters( 'mwb_wcwl_is_product_in_wishlist', $wishlist->has_product( $product_id ), $product_id, $wishlist_id );
		}
		 ...
		 * ]
		 *
		 * @return mwb_WCWL_Wishlist_Item[]|bool
		 * @since 2.0.0
		 */
		public function get_products( $args = array() ) {
			return mwb_WCWL_Wishlist_Factory::get_wishlist_items( $args );
		}
		...
		public function count_products( $wishlist_token = false ) {
		   $wishlist = mwb_WCWL_Wishlist_Factory::get_wishlist( $wishlist_token );
		...
		public function count_all_products() {
			...
			if( is_user_logged_in() ){
				...
				$id = mwb_WCWL_Session()->get_session_id();
				$args['session_id'] = $id;
			}

			if( false === $count = wp_cache_get( 'wishlist-user-total-count-' . $id, 'wishlists' ) ) {
				$count = mwb_WCWL_Wishlist_Factory::get_wishlist_items_count( $args );
		...
		public function count_add_to_wishlist( $product_id = false ) {
			...
			$product_id = ! ( $product_id ) ? mwb_get_product_id( $product ) : $product_id;

			if( ! $product_id ){
				return 0;
			}

			$count = mwb_WCWL_Wishlist_Factory::get_times_added_count( $product_id );
		...
		public function count_product_occurrences( $product_id = false ) {
			...
			$count = mwb_WCWL_Wishlist_Factory::get_wishlist_items_count( array( 'product_id' => $product_id, 'user_id' => false, 'session_id' => false, 'wishlist_id' => 'all' ) );
		...
		public function get_wishlists( $args = array() ){
			return mwb_WCWL_Wishlist_Factory::get_wishlists( $args );
		}

		/**
		 * Wrapper for \mwb_WCWL::get_wishlists, will return wishlists for current user
		 *
		 * @return mwb_WCWL_Wishlist[]
		 * @since 2.0.0
		 */
		public function get_current_user_wishlists() {
			$id = is_user_logged_in() ? get_current_user_id() : mwb_WCWL_Session()->get_session_id();

			$lists = wp_cache_get( 'user-wishlists-' . $id, 'wishlists' );

			if ( ! $lists ) {
				$lists = mwb_WCWL_Wishlist_Factory::get_wishlists(
					array(
						'orderby' => 'dateadded',
						'order' => 'ASC',
					)
		...
		public function get_wishlist_detail_by_token( $wishlist_token ) {
			return mwb_WCWL_Wishlist_Factory::get_wishlist( $wishlist_token );
		}
		...
		public function generate_default_wishlist( $id = false ){
			$wishlist = mwb_WCWL_Wishlist_Factory::generate_default_wishlist( $id );
		...
		public function generate_wishlist_token(){
			return mwb_WCWL_Wishlist_Factory::generate_wishlist_token();
		}
		...
		public function get_users_with_wishlist( $args = array() ){
			return mwb_WCWL_Wishlist_Factory::get_wishlist_users( $args );
		}
		 ...
		 *
		 * TODO: merge this into \mwb_WCWL_Wishlist::current_user_can
		 *
		...
		public function can_user_add_to_wishlist( $user_id = false ) {
			...
			$disable_wishlist_for_unauthenticated_users = get_option( 'mwb_wcwl_disable_wishlist_for_unauthenticated_users' );
			$return = true;
			...
			return apply_filters( 'mwb_wcwl_can_user_add_to_wishlist', $return, $user_id );
		}
		...
		public function register_data_stores( $data_stores ) {
			$data_stores['wishlist'] = 'mwb_WCWL_Wishlist_Data_Store';
			$data_stores['wishlist-item'] = 'mwb_WCWL_Wishlist_Item_Data_Store';
		...
		public function add_rewrite_rules() {
			...
			$this->wishlist_param = apply_filters( 'mwb_wcwl_wishlist_param', $this->wishlist_param );

			$wishlist_page_id = isset( $_POST['mwb_wcwl_wishlist_page_id'] ) ? $_POST['mwb_wcwl_wishlist_page_id'] : get_option( 'mwb_wcwl_wishlist_page_id' );
			$wishlist_page_id = mwb_wcwl_object_id( $wishlist_page_id, 'page', true, 'default' );
		...
		public function get_wishlist_page_id() {
			$wishlist_page_id = get_option( 'mwb_wcwl_wishlist_page_id' );
			$wishlist_page_id = mwb_wcwl_object_id( $wishlist_page_id );

			return apply_filters( 'mwb_wcwl_wishlist_page_id', $wishlist_page_id );
		}
		...
		public function get_wishlist_url( $action = '' ) {
			...
			return apply_filters( 'mwb_wcwl_wishlist_page_url', esc_url_raw( $base_url ), $action );
		}
		...
		public function get_add_to_wishlist_url( $product_id, $args = array() ) {
			...
			return apply_filters( 'mwb_wcwl_add_to_wishlist_url', esc_url_raw( $url ), $product_id, $args );
		}
		...
		public function get_available_views() {
			$available_views = apply_filters( 'mwb_wcwl_available_wishlist_views', array( 'view', 'user' ) );
		...
		public function get_pll_wishlist_url( $url ) {
			if( mwb_wcwl_is_wishlist_page() && isset( $_GET[ $this->wishlist_param ] ) ) {
...
/**
 * Unique access to instance of mwb_WCWL class
 *
 * @return \mwb_WCWL|\mwb_WCWL_Premium
 * @since 2.0.0
 */
function mwb_WCWL(){
	return defined( 'mwb_WCWL_PREMIUM' ) ? mwb_WCWL_Premium::get_instance() : mwb_WCWL::get_instance();
}
