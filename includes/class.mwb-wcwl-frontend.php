...
if ( ! class_exists( 'mwb_WCWL_Frontend' ) ) {
	/**
	 * Initiator class. Install the plugin database and load all needed stuffs.
	 *
	 * @since 1.0.0
	 */
	class mwb_WCWL_Frontend {

		/**
		 * Single instance of the class
		 *
		 * @var \mwb_WCWL_Frontend
		 * @since 2.0.0
		 */
		...
		/**
		 * Store class mwb_WCWL_Install.
		 *
		 * @var object
		 * @access private
		 * @since 1.0.0
		 */
		protected $_mwb_wcwl_install;
		...
		public function __construct() {
			define( 'mwb_WCWL_VERSION', $this->version );
			define( 'mwb_WCWL_DB_VERSION', $this->db_version );

			$this->_mwb_wcwl_install = mwb_WCWL_Install();

			// add filter for font-awesome compatibility
			add_filter( 'option_mwb_wcwl_add_to_wishlist_icon', array( $this, 'update_font_awesome_classes' ) );
			add_filter( 'option_mwb_wcwl_add_to_cart_icon', array( $this, 'update_font_awesome_classes' ) );
			...
			add_action( 'mwb_wcwl_before_wishlist_title', array( $this, 'print_notices' ) );
			add_action( 'mwb_wcwl_wishlist_before_wishlist_content', array( $this, 'wishlist_header' ), 10, 1 );
			add_action( 'mwb_wcwl_wishlist_main_wishlist_content', array( $this, 'main_wishlist_content' ), 10, 1 );
			add_action( 'mwb_wcwl_wishlist_after_wishlist_content', array( $this, 'wishlist_footer' ), 10, 1 );
			...
			// add mwb WooCommerce Frequently Bought Together Premium shortcode
			add_action( 'mwb_wcwl_after_wishlist_form', array( $this, 'mwb_wcfbt_shortcode' ), 10, 1 );
			add_filter( 'woocommerce_add_to_cart_redirect', array( $this, 'mwb_wfbt_redirect_after_add_to_cart' ), 10, 1 );

			// mwb WCWL Loaded
			do_action( 'mwb_wcwl_loaded' );
		}
		...
		public function add_button() {
			$positions = apply_filters( 'mwb_wcwl_positions', array(
			...
			$position = get_option( 'mwb_wcwl_button_position', 'add-to-cart' );
			...
			$enabled_on_loop = 'yes' == get_option( 'mwb_wcwl_show_on_loop', 'no' );

			if( ! $enabled_on_loop ){
				return;
			}

			$positions = apply_filters( 'mwb_wcwl_loop_positions', array(
			...
			// Add the link "Add to wishlist"
			$position = get_option( 'mwb_wcwl_loop_position', 'after_add_to_cart' );
		...
		public function print_button() {
			 ...
			 * @since 3.0.7
			 */
			if( ! apply_filters( 'mwb_wcwl_show_add_to_wishlist', true ) ){
				return;
			}

			echo do_shortcode( "[mwb_wcwl_add_to_wishlist]" );
		}
		...
		public function add_body_class( $classes ) {
			$wishlist_page_id = mwb_WCWL()->get_wishlist_page_id();
		...
		public function add_nocache_headers() {
			if( ! headers_sent() && mwb_wcwl_is_wishlist_page() ){
				wc_nocache_headers();
			}
		}
		...
		public function register_styles() {
			...
			wp_register_style( 'jquery-selectBox', mwb_WCWL_URL . 'assets/css/jquery.selectBox.css', array(), '1.2.0' );
			wp_register_style( 'mwb-wcwl-font-awesome', mwb_WCWL_URL . 'assets/css/font-awesome.css', array(), '4.7.0' );
			...
			if( ! $located ){
				wp_register_style( 'mwb-wcwl-main', mwb_WCWL_URL . 'assets/css/style.css', array( 'jquery-selectBox', 'mwb-wcwl-font-awesome' ), $this->version );
			}
			else{
				...
				wp_register_style( 'mwb-wcwl-user-main', $style_url, array( 'jquery-selectBox', 'mwb-wcwl-font-awesome' ), $this->version );
			}
			...
			if( $current_theme->exists() ){
				...
				if( file_exists( mwb_WCWL_DIR . 'assets/css/themes/' . $theme_slug . '.css' ) ){
					wp_register_style( 'mwb-wcwl-theme', mwb_WCWL_URL . 'assets/css/themes/' . $theme_slug . '.css', array( $located ? 'mwb-wcwl-user-main' : 'mwb-wcwl-main' ), $this->version );
				}
			}
		}
		...
		public function register_scripts() {
			...
			wp_register_script( 'jquery-selectBox', mwb_WCWL_URL . 'assets/js/jquery.selectBox.min.js', array( 'jquery' ), '1.2.0', true );

			$mwb_wcwl_l10n = $this->get_localize();
			$main_script_deps = apply_filters( 'mwb_wcwl_main_script_deps', array( 'jquery', 'jquery-selectBox' ) );

			if ( ! $located ) {
				wp_register_script( 'jquery-mwb-wcwl', mwb_WCWL_URL . 'assets/js/' . $prefix . 'jquery.mwb-wcwl.js', $main_script_deps, $this->version, true );
				wp_localize_script( 'jquery-mwb-wcwl', 'mwb_wcwl_l10n', $mwb_wcwl_l10n );
			}
			else {
				wp_register_script( 'jquery-mwb-wcwl-user', str_replace( get_stylesheet_directory(), get_stylesheet_directory_uri(), $located ), $main_script_deps, $this->version, true );
				wp_localize_script( 'jquery-mwb-wcwl-user', 'mwb_wcwl_l10n', $mwb_wcwl_l10n );
			}
		}
		...
		public function enqueue_styles_and_stuffs() {
			...
			wp_enqueue_style( 'mwb-wcwl-font-awesome' );

			// main plugin style
			if ( ! wp_style_is( 'mwb-wcwl-user-main', 'registered' ) ) {
				wp_enqueue_style( 'mwb-wcwl-main' );
			}
			else {
				wp_enqueue_style( 'mwb-wcwl-user-main' );
			}

			// theme specific style
			if( wp_style_is( 'mwb-wcwl-theme', 'registered' ) ){
				wp_enqueue_style( 'mwb-wcwl-theme' );;
			}
		...
		public function enqueue_custom_style() {
			...
			if( $custom_css ) {
				$handle = wp_script_is( 'mwb-wcwl-user-main' ) ? 'mwb-wcwl-user-main' : 'mwb-wcwl-main';
		...
		public function enqueue_scripts() {
			...
			if ( ! wp_script_is( 'jquery-mwb-wcwl-user', 'registered' ) ) {
				wp_enqueue_script( 'jquery-mwb-wcwl' );
			}
			else {
				wp_enqueue_script( 'jquery-mwb-wcwl-user' );
			}
		}
		...
		public function get_localize() {
			return apply_filters( 'mwb_wcwl_localize_script', array(
				'ajax_url' => admin_url( 'admin-ajax.php', 'relative' ),
				'redirect_to_cart' => get_option( 'mwb_wcwl_redirect_cart' ),
				'multi_wishlist' => false,
				'hide_add_button' => apply_filters( 'mwb_wcwl_hide_add_button', true ),
				'enable_ajax_loading' => 'yes' == get_option( 'mwb_wcwl_ajax_enable', 'no' ),
				'ajax_loader_url' => mwb_WCWL_URL . 'assets/images/ajax-loader-alt.svg',
				'remove_from_wishlist_after_add_to_cart' => get_option( 'mwb_wcwl_remove_after_add_to_cart' ) == 'yes',
				'is_wishlist_responsive' => apply_filters( 'mwb_wcwl_is_wishlist_responsive', true ),
				'time_to_close_prettyphoto' => apply_filters( 'mwb_wcwl_time_to_close_prettyphoto', 3000 ),
				'fragments_index_glue' => apply_filters( 'mwb_wcwl_fragments_index_glue', '.' ),
				'labels' => array(
					'cookie_disabled' => __( 'We are sorry, but this feature is available only if cookies on your browser are enabled.', 'mwb-woocommerce-wishlist' ),
					'added_to_cart_message' => sprintf( '<div class="woocommerce-notices-wrapper"><div class="woocommerce-message" role="alert">%s</div></div>', apply_filters( 'mwb_wcwl_added_to_cart_message', __( 'Product added to cart successfully', 'mwb-woocommerce-wishlist' ) ) )
				),
				'actions' => array(
		...
		public function main_wishlist_content( $var ) {
			...
			mwb_wcwl_get_template_part( $template, '', $layout, $var );
		}
		...
		public function wishlist_header( $var ) {
			...
			mwb_wcwl_get_template_part( $template, 'header', $layout, $var );
		}
		...
		public function wishlist_footer( $var ) {
			...
			mwb_wcwl_get_template_part( $template, 'footer', $layout, $var );
		}
		...
		public function alter_add_to_cart_args( $args ) {
			$use_custom_button = get_option( 'mwb_wcwl_add_to_cart_style' );
			$button_class = in_array( $use_custom_button, array( 'button_custom', 'button_default' ) );
			$icon = get_option( 'mwb_wcwl_add_to_cart_icon' );
			$custom_icon = get_option( 'mwb_wcwl_add_to_cart_custom_icon' );
		...
		public function alter_add_to_cart_text( $text, $product ) {
			$label_option = get_option( 'mwb_wcwl_add_to_cart_text', __( 'Add to cart', 'mwb-woocommerce-wishlist' ) );
			$label = $product->is_type( 'variable' ) ? $text : apply_filters( 'mwb_wcwl_add_to_cart_label', $label_option );
		...
		public function alter_add_to_cart_url( $url, $product ) {
			global $mwb_wcwl_wishlist_token;

			if( $mwb_wcwl_wishlist_token ){
				$wishlist = mwb_wcwl_get_wishlist( $mwb_wcwl_wishlist_token );
				...
				if( wp_doing_ajax() ){
					$url = add_query_arg( 'add-to-cart', $product->get_id(), mwb_WCWL()->get_wishlist_url( 'view/' .  $mwb_wcwl_wishlist_token ) );
				}

				if( $product->is_type( array( 'simple', 'variation' ) ) && get_option( 'mwb_wcwl_redirect_cart' ) == 'yes' ){
					$url = add_query_arg( 'add-to-cart', $product->get_id(), wc_get_cart_url() );
				}

				if( ! $product->is_type( 'external' ) && get_option( 'mwb_wcwl_remove_after_add_to_cart' ) == 'yes' ){
					$url = add_query_arg(
						array(
							'remove_from_wishlist_after_add_to_cart' => $product->get_id(),
							'wishlist_id' => $wishlist_id,
							'wishlist_token' => $mwb_wcwl_wishlist_token
						),
						$url
					);
				}

				if( $item && 'yes' == get_option( 'mwb_wcwl_quantity_show' ) ){
					$url = add_query_arg( 'quantity', $item->get_quantity(), $url );
				}
			}

			return apply_filters( 'mwb_wcwl_add_to_cart_redirect_url', esc_url_raw( $url ), $url, $product );
		}
		 ...
		 */
		public function mwb_wcfbt_shortcode( $meta ){

			if( ! ( defined( 'mwb_WFBT' ) && mwb_WFBT ) || get_option( 'mwb_wfbt_enable_integration' ) == 'no' ) {
				return;
			}

			$products = mwb_WCWL()->get_products(
				array(
			...
			do_shortcode( '[mwb_wfbt products="' . implode( ',', $ids ) . '"]' );
		}

		/**
		 * Redirect after add to cart from mwb WooCommerce Frequently Bought Together Premium shortcode
		 *
		 * @since 2.0.0
		 */
		public function mwb_wfbt_redirect_after_add_to_cart( $url ){
			if( ! isset( $_REQUEST['mwb_wfbt_shortcode'] ) ) {
				return $url;
			}

			return get_option( 'mwb_wcwl_redirect_cart' ) == 'yes' ? wc_get_cart_url() : mwb_WCWL()->get_wishlist_url();
		}
		...
		protected function _build_custom_css( $rules = array() ){
			$generated_code = '';
			$rules = apply_filters( 'mwb_wcwl_custom_css_rules', array_merge( array(
				'color_add_to_wishlist' => array(
					...
					'deps' => array(
						'mwb_wcwl_add_to_wishlist_style' => 'button_custom'
					)
				),
				...
				'color_add_to_cart' => array(
					...
					'deps' => array(
						'mwb_wcwl_add_to_cart_style' => 'button_custom'
					)
				),
				...
				'color_button_style_1' => array(
					'selector' => '.woocommerce .hidden-title-form button,
								   .mwb-wcwl-wishlist-new .create-wishlist-button,
								   .wishlist_manage_table tfoot button.submit-wishlist-changes,
								   .mwb-wcwl-wishlist-search-form button.wishlist-search-button',
					'rules'    => array(
						'background' => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#333333'
				...
				'color_headers_background' => array(
					'selector' => '.wishlist_table thead tr th,
								   ...
								   .widget_mwb-wcwl-lists ul.dropdown li.current a,
								   .widget_mwb-wcwl-lists ul.dropdown li a:hover,
				...
				'color_share_button' => array(
					...
					'deps' => array(
						'mwb_wcwl_enable_share' => 'yes'
					)
				),
				'color_fb_button' => array(
					'selector' => '.mwb-wcwl-share a.facebook',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#39599E'
					...
					'deps' => array(
						'mwb_wcwl_enable_share' => 'yes',
						'mwb_wcwl_share_fb' => 'yes',
					)
				),
				'color_tw_button' => array(
					'selector' => '.mwb-wcwl-share a.twitter',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#45AFE2'
					...
					'deps' => array(
						'mwb_wcwl_enable_share' => 'yes',
						'mwb_wcwl_share_twitter' => 'yes',
					)
				),
				'color_pr_button' => array(
					'selector' => '.mwb-wcwl-share a.pinterest',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#AB2E31'
					...
					'deps' => array(
						'mwb_wcwl_enable_share' => 'yes',
						'mwb_wcwl_share_pinterest' => 'yes',
					)
				),
				'color_em_button' => array(
					'selector' => '.mwb-wcwl-share a.email',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#FBB102'
					...
					'deps' => array(
						'mwb_wcwl_enable_share' => 'yes',
						'mwb_wcwl_share_email' => 'yes',
					)
				),
				'color_wa_button' => array(
					'selector' => '.mwb-wcwl-share a.whatsapp',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#00A901'
			...
			foreach( $rules as $id => $rule ){
				...
				$values = get_option( "mwb_wcwl_{$id}" );
				$new_rules = array();
				$rules_code = '';
			...
			if( $custom_css = get_option( 'mwb_wcwl_custom_css' ) ){
		...
		protected function _update_cookies(){
			$cookie = mwb_getcookie( 'mwb_wcwl_products' );
			$new_cookie = array();
			...
			if( ! empty( $cookie ) ) {
				...
				mwb_setcookie( 'mwb_wcwl_products', $new_cookie );
			}
		}
		...
		protected function _convert_cookies_to_session(){
			...
			if( ! empty( $cookie ) ){
				...
				$default_list = mwb_WCWL_Wishlist_Factory::get_default_wishlist();
				...
				foreach ( $cookie as $item ){
					...
					$new_item = new mwb_WCWL_Wishlist_Item();
				...
				mwb_destroycookie( 'mwb_wcwl_products' );
            }
		}
	}
}
...
/**
 * Unique access to instance of mwb_WCWL_Frontend class
 *
 * @return \mwb_WCWL_Frontend|\mwb_WCWL_Frontend_Premium
 * @since 2.0.0
 */
function mwb_WCWL_Frontend(){
	return defined( 'mwb_WCWL_PREMIUM' ) ? mwb_WCWL_Frontend_Premium::get_instance() : mwb_WCWL_Frontend::get_instance();
}
