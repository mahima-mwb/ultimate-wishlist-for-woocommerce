...
if ( ! class_exists( 'mwb_WCWL_Wishlist_Item' ) ) {
	/**
	 * This class describes Wishlist Item object, and it is meant to be created by mwb_WCWL_Wishlist class, via
	 * get_items method
	 *
	 * @since 3.0.0
	 */
	class mwb_WCWL_Wishlist_Item extends WC_Data implements ArrayAccess {
		 ...
		 * Constructor.
		 *
		 * @param int|object|array $item ID to load from the DB, or mwb_WCWL_Wishlist_Item object.
		...
		public function get_product_id( $context = 'view' ) {
			return mwb_wpml_object_id( $this->get_original_product_id( $context ), 'product', true );
		}
		...
		public function get_formatted_product_price( $context = 'view' ) {
			...
			$formatted_price = $base_price ? $product->get_price_html() : apply_filters( 'mwb_free_text', __( 'Free!', 'mwb-woocommerce-wishlist' ), $product );

			return apply_filters( 'mwb_wcwl_item_formatted_price', $formatted_price, $base_price, $product );
		}
		...
		public function get_price_variation() {
			...
			if( $difference <= 0 && apply_filters( 'mwb_wcwl_hide_price_increase', true, $product, $original_price, $original_currency ) ){
				return '';
			}
			...
			$template = apply_filters( 'mwb_wcwl_price_variation_template', sprintf(
				'<small class="price-variation %s"><span class="variation-rate">%s</span><span class="old-price">%s</span></small>',
				$class,
				_x( 'Price is %1$s%%', 'Part of the template that shows price variation since addition to list; placeholder will be replaced with a percentage', 'mwb-woocommerce-wishlist' ),
				_x( '(Was %2$s when added  in list)', 'Part of the template that shows price variation since addition to list; placeholder will be replaced with a price', 'mwb-woocommerce-wishlist' )
		...
		public function set_wishlist_id( $wishlist_id ) {
			...
			$wishlist = mwb_wcwl_get_wishlist( $wishlist_id );
		...
		public function set_on_sale( $on_sale ) {
			if( $this->get_object_read() && $on_sale && $this->is_on_sale() != $on_sale ){
				do_action( 'mwb_wcwl_item_is_on_sale', $this );
			}
		...
		protected function map_legacy_offsets( $offset ) {
			...
			return apply_filters( 'mwb_wcwl_wishlist_item_map_legacy_offsets', $offset, $legacy_offset );
		}
	}
}
