...
var minifyMainJs = function () {
    return gulp.src('./assets/js/unminified/jquery.mwb-wcwl.js')
        .pipe(uglify())
        .pipe(gulp.dest('./assets/js/'))
};

var minifyAdminJs = function () {
    return gulp.src('./assets/js/unminified/admin/mwb-wcwl.js')
        .pipe(uglify())
...
var updatePot = function () {
    return gulp.src(['./*.php', './**/*.php', '!./plugin-fw/**/*.php', '!./plugin-upgrade/**/*.php'])
        .pipe(wpPot( {
            domain: 'mwb-woocommerce-wishlist',
            package: 'mwb WooCommerce Wishlist',
            destFile: './languages/mwb-woocommerce-wishlist.pot',
            metadataFile: '../init.php',
            headers: {
                "Project-Id-Version": "mwb WooCommerce Wishlist Premium",
                "Content-Type": "text/plain; charset=UTF-8",
                "Language-Team": "mwb <plugins@mwbemes.com>",
        ...
        .pipe(gulp.dest('./languages/mwb-woocommerce-wishlist.pot'));
};

var updatePo = function () {
    return gulp.src('./languages/**/*.po')
        .pipe(poSync('./languages/mwb-woocommerce-wishlist.pot'))
...
var validateJs = function () {
    return gulp.src('./assets/js/unminified/*mwb*.js')
        .pipe(jshint())
