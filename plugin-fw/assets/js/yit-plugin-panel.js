...
jQuery( function ( $ ) {
    ...
    $( '[data-dep-target]' ).each( function () {
        ...
        if( field_type === 'select-images' ){
          event = 'mwb_select_images_value_changed';
        }
    ...
    function dependencies_handler( id, deps, values, type ) {
        ...
        var $current_field     = $( id ),
            $current_container = $( id + '-container' ).closest( 'tr' ); // container for mwb Plugin Panel

        if ( $current_container.length < 1 ) {
            // container for mwb Plugin Panel WooCommerce
            $current_container = $current_field.closest( '.mwb-plugin-fw-panel-wc-row' );
        }
        ...
        for ( j in types ) {
            ...
            if ( !result ) {
                switch ( current_type ) {
                    case 'disable':
                        $current_container.addClass( 'mwb-disabled' );
                ...
                switch ( current_type ) {
                    case 'disable':
                        $current_container.removeClass( 'mwb-disabled' );
    ...
    $( '.mwb-plugin-fw-panel .woo-nav-tab-wrapper' ).removeClass( 'woo-nav-tab-wrapper' ).addClass( 'mwb-nav-tab-wrapper' );

    var wrap    = $( '.wrap.mwb-plugin-ui' ).first(),
    ...
    var active_subnav = $(document).find( '.mwb-nav-sub-tab.nav-tab-active' );

    if( active_subnav.length ){
        // WP page
        var  mainWrapper = $(document).find( '.mwb-plugin-fw-wp-page-wrapper' );
        if( ! mainWrapper.length ){
            mainWrapper = $(document).find( '#wpbody-content > .mwb-plugin-ui' );
        }

        if( mainWrapper ){
            // serach first for deafult wrap
            var wrap = mainWrapper.find( '.mwb-admin-panel-content-wrap' );
            if( wrap.length ) {
