<?php
/**
 * Framework Name: mwb Plugin Framework
 * Version: 3.4.31
 * Author: mwb
...
if ( ! function_exists ( 'mwb_maybe_plugin_fw_loader' ) ) {
    /**
     * mwb_maybe_plugin_fw_loader
     *
     * @since 1.0.0
     */
    function mwb_maybe_plugin_fw_loader ( $plugin_path ) {
	    ...
	    $plugin_fw_main_file = $plugin_path . 'plugin-fw/mwb-plugin.php';
	    ...
	    if( file_exists( $plugin_upgrade_path ) ){
		    if( ! empty( $plugin_upgrade_fw_data ) ){
				foreach( $plugin_upgrade_fw_data as $version => $files ){
					if( version_compare ( $version, $framework_data[ 'Version' ], '<' ) ){
						$plugin_upgrade_fw_data = array ( $framework_data[ 'Version' ] => mwb_get_upgrade_files( $plugin_license_path, $plugin_upgrade_path ) );
					}
				}
		    }
...
if( ! function_exists( 'mwb_get_upgrade_files' ) ){
	/**
	 ...
	 */
	function mwb_get_upgrade_files( $plugin_license_path, $plugin_upgrade_path = '' ){
		$to_include = array();

		if( false ==! $plugin_license_path ){
			$plugin_upgrade_path = empty( $plugin_upgrade_path ) ? $plugin_license_path : $plugin_upgrade_path;
			$license_files = array(
				'%mwb-license-path%/lib/mwb-licence.php',
				'%mwb-license-path%/lib/mwb-plugin-licence.php',
				'%mwb-license-path%/lib/mwb-theme-licence.php',
			);

			$upgrade_files = array( '%mwb-upgrade-path%/lib/mwb-plugin-upgrade.php' );

			$to_include_license = str_replace( '%mwb-license-path%', $plugin_license_path, $license_files );
			$to_include_upgrade = str_replace( '%mwb-upgrade-path%', $plugin_upgrade_path, $upgrade_files );
