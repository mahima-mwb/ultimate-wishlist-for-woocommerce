...
$current_icon    = mwb_Icon()->get_icon_data( $current_options[ 'icon' ] );
$std_icon        = mwb_Icon()->get_icon_data( $std[ 'icon' ] );

$options[ 'icon' ] = mwb_Plugin_Common::get_icon_list();

?>


<div id="<?php echo $id ?>-container" class="select_icon rm_option rm_input rm_text" <?php echo mwb_field_deps_data( $args ); ?>>
    <div id="<?php echo $id ?>-container" <?php echo mwb_field_deps_data( $args ); ?>>
        ...
        <div class="option">
            ...
            <div class="input_wrapper custom_icon_wrapper upload" style="clear:both;">
                <input type="text" name="<?php echo $name ?>[custom]" id="<?php echo $id ?>[custom]" value="<?php echo $current_options[ 'custom' ] ?>" class="mwb-plugin-fw-upload-img-url upload_custom_icon"/>
                <input type="button" value="<?php _e( 'Upload', 'mwb-plugin-fw' ) ?>" id="<?php echo $id; ?>-custom-button" class="mwb-plugin-fw-upload-button button"/>

                <div class="mwb-plugin-fw-upload-img-preview" style="margin-top:10px;">
                    <?php
                    $file = $current_options[ 'custom' ];
                    if ( preg_match( '/(jpg|jpeg|png|gif|ico)$/', $file ) ) {
                        echo __( 'Image preview', 'mwb-plugin-fw' ) . ': ' . "<img src=\"" . mwb_CORE_ASSETS_URL . "/images/sleep.png\" data-src=\"$file\" />";
                    }
                    ?>
                </div>
            </div>
        ...
        <div class="description">
            ...
            <?php if ( $std[ 'select' ] == 'custom' ) : ?>
                <?php printf( __( '(Default: %s <img src="%s"/>)', 'mwb-plugin-fw' ), $options[ 'select' ][ 'custom' ], $std[ 'custom' ] ) ?>
            <?php else: ?>
                <?php printf( __( '(Default: <i %s></i> )', 'mwb-plugin-fw' ), $std_icon ) ?>
            <?php endif; ?>
        </div>
