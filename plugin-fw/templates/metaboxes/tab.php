...
global $post;
$classes = apply_filters( 'mwb_plugin_fw_metabox_class', $class, $post );
$classes = mwb_plugin_fw_remove_duplicate_classes( $classes );

do_action( 'mwb_before_metaboxes_tab' ) ?>
<div class="mwb-plugin-fw metaboxes-tab <?php echo esc_attr( $classes ) ?>">
    <?php do_action( 'mwb_before_metaboxes_labels' ) ?>
    <ul class="metaboxes-tabs clearfix"<?php if ( count( $tabs ) <= 1 ) : ?> style="display:none;"<?php endif; ?>>
        ...
        foreach ( $tabs as $key => $tab ) :
            ...
            $anchor_id = 'mwb-plugin-fw-metabox-tab-' . urldecode( $key ) . '-anchor';
            ...
            ?>
        <li id="<?php echo $anchor_id ?>" <?php if ( !$i ) : ?>class="tabs"<?php endif ?> <?php echo mwb_field_deps_data( $tab ); ?>>
            <a href="#<?php echo urldecode( $key ) ?>"><?php echo $tab[ 'label' ] ?></a></li><?php
            $i++;
        endforeach;
        ?>
    </ul>
    <?php do_action( 'mwb_after_metaboxes_labels' ) ?>
    <?php if ( isset( $tab[ 'label' ] ) ) : ?>
        <?php do_action( 'mwb_before_metabox_option_' . urldecode( $key ) ); ?>
    <?php endif ?>

    <?php
    // Use nonce for verification
    wp_nonce_field( 'metaboxes-fields-nonce', 'mwb_metaboxes_nonce' );
    ?>
    <?php foreach ( $tabs as $key => $tab ) :
        ...
        <div class="tabs-panel" id="<?php echo urldecode( $key ) ?>">
            ...
            $tab[ 'fields' ] = apply_filters( 'mwb_metabox_' . $key . '_tab_fields', $tab[ 'fields' ] );

            foreach ( $tab[ 'fields' ] as $id_tab => $field ) :
                $field_name = $field[ 'name' ];
                $field_name  = str_replace( 'mwb_metaboxes[', '', $field_name );
                if ( $pos = strpos( $field_name, ']' ) ) {
                    $field_name = substr_replace( $field_name, '', $pos, 1 );
                }
				$value                  = mwb_get_post_meta( $post->ID, $field_name );
                ...
                <div class="<?php echo $container_classes ?>">
                    <?php
                    if ( $field_template_path = mwb_plugin_fw_get_field_template_path( $field ) ) {
                        $display_row                   = 'hidden' !== $field[ 'type' ];
                        $display_row                   = isset( $field[ 'mwb-display-row' ] ) ? !!$field[ 'mwb-display-row' ] : $display_row;
                        $field[ 'display-field-only' ] = in_array( $field[ 'type' ], array( 'hidden', 'html', 'sep', 'simple-text', 'title', 'list-table' ) );

                        if ( $display_row ) {

                            $field_row_path = apply_filters( 'mwb_plugin_fw_metabox_field_row_template_path', mwb_CORE_PLUGIN_TEMPLATE_PATH . '/metaboxes/field-row.php', $field );
                        ...
                        $args = apply_filters( 'mwb_fw_metaboxes_type_args', array(
                                                                               'basename' => mwb_CORE_PLUGIN_PATH,
                        ...
                        extract( $args );
                        mwb_plugin_get_template( $basename, $path . $type . '.php', $args );
                    }
                    ?>
                </div>
            <?php endforeach ?>
...
<?php do_action( 'mwb_after_metaboxes_tab' );