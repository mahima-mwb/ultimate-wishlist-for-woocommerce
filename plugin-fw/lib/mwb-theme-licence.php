...
if ( ! class_exists( 'mwb_Theme_Licence' ) ) {
	/**
	 * mwb Licence Panel
	 *
	...
	class mwb_Theme_Licence {
		...
		public function register( $init, $secret_key, $product_id ){
			if( ! function_exists( 'mwb_Theme_Licence' ) ){
				//Try to load mwb_Theme_Licence class
				mwb_plugin_fw_load_update_and_licence_files();
			}

            try {
                mwb_Theme_Licence()->register( $init, $secret_key, $product_id );
            } catch( Error $e ){
            }
		}
 ...
 */
if ( !function_exists( 'mwb_Theme_Licence' ) ) {
	function mwb_Theme_Licence() {
		return mwb_Theme_Licence::instance();
	}
}