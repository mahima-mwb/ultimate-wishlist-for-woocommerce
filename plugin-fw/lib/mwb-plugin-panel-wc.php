...
if ( !class_exists( 'mwb_Plugin_Panel_WooCommerce' ) ) {
    /**
     * mwb Plugin Panel for WooCommerce
     ...
     */
    class mwb_Plugin_Panel_WooCommerce extends mwb_Plugin_Panel {
         ...
         * @var array a setting list of parameters
         */
        public static $body_class = ' mwb-plugin-fw-panel ';
        ...
        public function __construct( $args = array() ) {
            ...
            $args = apply_filters( 'mwb_plugin_fw_wc_panel_option_args', $args );
            if ( !empty( $args ) ) {
                ...
                //mwb-plugin-ui
                add_action( 'mwb_plugin_fw_get_field_after', array( $this, 'add_mwb_ui' ) );
                add_action( 'mwb_plugin_fw_before_woocommerce_panel', array( $this, 'add_plugin_banner' ), 10, 1 );
                add_action( 'admin_action_mwb_plugin_fw_save_toggle_element', array( $this, 'save_toggle_element_options' ) );
        ...
        protected static function _init_actions() {
            if ( !static::$_actions_initialized ) {
                ...
                /* Add mwb Fields */
                add_action( 'woocommerce_admin_field_mwb-field', array( __CLASS__, 'add_mwb_field' ), 10, 1 );
        ...
        public function mwb_panel() {
            ...
            $additional_info                      = apply_filters( 'mwb_admin_tab_params', $additional_info );
            $additional_info[ 'additional_info' ] = $additional_info;

            extract( $additional_info );
            require_once( mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/woocommerce/woocommerce-panel.php' );
        }
         ...
         */
        public function mwb_upload_update( $option_value ) {
            return $option_value;
        }
         ...
         * @author   Emanuela Castorina      <emanuela.castorina@mwbemes.com>
         */
        public function mwb_upload( $args = array() ) {
            if ( !empty( $args ) ) {
                $args[ 'value' ] = ( get_option( $args[ 'id' ] ) ) ? get_option( $args[ 'id' ] ) : $args[ 'default' ];
                extract( $args );

                include( mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/woocommerce/woocommerce-upload.php' );
            }
        }
         ...
         * @return mixed
         * @since    1.0.0
         * @author   Antonino Scarfì      <antonino.scarfi@mwbemes.com>
         */
        ...
        public function get_current_tab() {
	        ...
	        return apply_filters('mwb_wc_plugin_panel_current_tab', $tab );
        }
        ...
        public function add_fields() {
            ...
            woocommerce_admin_fields( $mwb_options[ $option_key ] );
        }
        ...
        public function print_panel_content() {
            ...
            if ( $custom_tab_action ) {
                ...
                require_once( mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/woocommerce/woocommerce-form.php' );
            }
        }
         ...
         * @internal fire two action (before and after update): mwb_panel_wc_before_update and mwb_panel_wc_after_update
         */
        public function woocommerce_update_options() {

            if ( isset( $_POST[ 'mwb_panel_wc_options_nonce' ] ) && wp_verify_nonce( $_POST[ 'mwb_panel_wc_options_nonce' ], 'mwb_panel_wc_options_' . $this->settings[ 'page' ] ) ) {

                do_action( 'mwb_panel_wc_before_update' );

                $mwb_options = $this->get_main_array_options();
                $option_key  = $this->get_current_option_key();
	            $mwb_options = $this->check_for_save_single_option( $mwb_options );

                if ( version_compare( WC()->version, '2.4.0', '>=' ) ) {
                    if ( !empty( $mwb_options[ $option_key ] ) ) {
                        foreach ( $mwb_options[ $option_key ] as $option ) {
                ...
                foreach ( $_POST as $name => $value ) {
                    ...
                    $attachment_id_needle = "-mwb-attachment-id";
                    ...
                    if ( $is_hidden_input ) {
                        ...
                        $mwb_options[ $option_key ][ $name ] = array(
                            "type" => "text",
                            "id"   => $name
                        );
                    }
	            ...
	            woocommerce_update_options( $mwb_options[ $option_key ] );

                do_action( 'mwb_panel_wc_after_update' );

            } elseif ( isset( $_REQUEST[ 'mwb-action' ] ) && $_REQUEST[ 'mwb-action' ] == 'wc-options-reset'
                       && isset( $_POST[ 'mwb_wc_reset_options_nonce' ] ) && wp_verify_nonce( $_POST[ 'mwb_wc_reset_options_nonce' ], 'mwb_wc_reset_options_' . $this->settings[ 'page' ] )
            ) {

                do_action( 'mwb_panel_wc_before_reset' );
                ...
                foreach ( $mwb_options[ $option_key ] as $id => $option ) {
	                ...
	                if( ! empty( $matches ) && isset( $option['default'] ) ) {
		                ...
		                if ( isset( $option['mwb-type'] ) && $option['mwb-type'] == 'multi-colorpicker' && ! empty( $option['colorpickers'] ) ) {
			                $default = [];
                ...
                do_action( 'mwb_panel_wc_after_reset' );
            }
        }
        ...
        public function admin_enqueue_scripts() {
            ...
            $assets_screen_ids = (array) apply_filters( 'mwb_plugin_fw_wc_panel_screen_ids_for_assets', array() );
            ...
            if ( $screen && ( 'admin.php' === $pagenow && strpos( $screen->id, $this->settings[ 'page' ] ) !== false ) || in_array( $screen->id, $assets_screen_ids ) ) {
                ...
                wp_localize_script( 'woocommerce_settings', 'woocommerce_settings_params', array(
                    'i18n_nav_warning' => __( 'The changes you have made will be lost if you leave this page.', 'mwb-plugin-fw' )
                ) );
                wp_enqueue_script( 'mwb-plugin-fw-fields' );
            }

            if ( $screen && ( 'admin.php' === $pagenow && mwb_plugin_fw_is_panel() ) || in_array( $screen->id, $assets_screen_ids ) ) {
                wp_enqueue_media();
                wp_enqueue_style( 'mwb-plugin-style' );
                wp_enqueue_script( 'mwb-plugin-panel' );
            }

            if ( $screen && 'admin.php' === $pagenow && strpos( $screen->id, 'mwb_upgrade_premium_version' ) !== false ) {
        ...
        public function set_default_options() {
            ...
            $page                = $this->settings[ 'page' ];
            $default_options_set = get_option( 'mwb_plugin_fw_panel_wc_default_options_set', array() );
            ...
            update_option( 'mwb_plugin_fw_panel_wc_default_options_set', $default_options_set );
        }
        ...
        public static function delete_default_options_set_option() {
            delete_option( 'mwb_plugin_fw_panel_wc_default_options_set' );
        }
		...
		public static function admin_body_class( $admin_body_classes ) {
			...
			if ( ( 'admin.php' == $pagenow && ( strpos( get_current_screen()->id, 'mwb-plugins_page' ) !== false || in_array( get_current_screen()->id, $assets_screen_ids ) ) ) ) {
        ...
        public function maybe_unserialize_panel_data( $value, $option, $raw_value ) {
            ...
            if ( !empty( $mwb_options[ $option_key ] ) ) {
                foreach ( $mwb_options[ $option_key ] as $option_array ) {
        ...
        public static function sanitize_option( $value, $option, $raw_value ) {
            ...
            if ( isset( $option[ 'type' ] ) && 'mwb-field' === $option[ 'type' ] ) {
                ...
                if ( isset( $option[ 'mwb-type' ] ) && in_array( $option[ 'mwb-type' ], array( 'checkbox', 'onoff' ) ) ) {
                    $value = mwb_plugin_fw_is_true( $raw_value ) ? 'yes' : 'no';
                }

                if ( isset( $option[ 'mwb-type' ] ) && 'checkbox-array' === $option[ 'mwb-type' ] ) {
                    $value = maybe_unserialize( $raw_value );
                    $value = !!$value && is_array( $value ) ? $value : array();
                }

                if ( isset( $option[ 'mwb-type' ] ) && in_array( $option[ 'mwb-type' ], array( 'textarea', 'textarea-editor', 'textarea-codemirror' ) ) ) {
                    $value = $raw_value;
                }

                // sanitize the option date-format when the user choose the custom option
                if ( isset( $option[ 'mwb-type' ] ) && in_array( $option[ 'mwb-type' ], array( 'date-format' ) ) && '\c\u\s\t\o\m' == $raw_value ) {
                    $custom = isset( $_REQUEST[ $option[ 'id' ] . '_text' ] ) ? $_REQUEST[ $option[ 'id' ] . '_text' ] : $option[ 'default' ];
                    $value  = $custom;
                }

                if ( isset( $option[ 'mwb-type' ] ) && in_array( $option[ 'mwb-type' ], array( 'toggle-element' ) ) ) {
                    ...
                    if ( $value && isset( $option[ 'elements' ] ) && !empty( $option[ 'elements' ] ) ) {
                        ...
                        foreach ( $value as $index => $single_toggle ) {
                            ...
                            if ( $value && isset( $option[ 'onoff_field' ] ) && !empty( $option[ 'onoff_field' ] ) ) {
                                ...
                                $onoff[ 'type' ]      = 'mwb-field';
                                $onoff[ 'mwb-type' ] = 'onoff';
                                $onoff_id             = $onoff[ 'id' ];
         ...
         * @author   Leanza Francesco <leanzafrancesco@gmail.com>
         */
        public static function add_mwb_field( $field = array() ) {
            if ( !empty( $field ) && isset( $field[ 'mwb-type' ] ) ) {
                $field[ 'type' ] = $field[ 'mwb-type' ];
                unset( $field[ 'mwb-type' ] );

                $field[ 'id' ]      = isset( $field[ 'id' ] ) ? $field[ 'id' ] : '';
                $field[ 'name' ]    = $field[ 'id' ];
                $field[ 'default' ] = isset( $field[ 'default' ] ) ? $field[ 'default' ] : '';

                $value = apply_filters( 'mwb_plugin_fw_wc_panel_pre_field_value', null, $field );
                if ( is_null( $value ) ) {
                ...
                $field = apply_filters( 'mwb_plugin_fw_wc_panel_field_data', $field );

                require( mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/woocommerce/woocommerce-option-row.php' );
            }
        }

        /**
         *  Save the content of the toggle element present inside the panel.
         *  Called by the action 'admin_action_mwb_plugin_fw_save_toggle_element'
         *  via Ajax
         *
        ...
        public function save_toggle_element_options() {
            ...
            if ( !empty( $mwb_options[ $current_tab ] ) && !empty( $option_id ) ) {

                $tab_options = $mwb_options[ $current_tab ];
                ...
                if ( isset( $option_array[ $option_id ] ) ) {
                    ...
                    //drag and drop
                    $order_elements = isset( $posted[ 'mwb_toggle_elements_order_keys' ] ) ? explode( ',', $posted[ 'mwb_toggle_elements_order_keys' ] ) : false;
                    if ( $order_elements ) {
                        $i         = 0;
                        $new_value = array();
                        foreach ( $order_elements as $key ) {
                            $index               = apply_filters( 'mwb_toggle_elements_index', $i++, $key );
	     ...
	     *
	     * @param $mwb_options mixed|array Original options array
	     *
	     * @return mixed|array New options array
	     * @author Andrea Grillo <andrea.grillo@mwbemes.com>
	     * @since  3.0.0
	     */
	    public function check_for_save_single_option( $mwb_options ){
		    foreach ( $mwb_options as $key => $options_list ){
			    foreach ( $options_list as $value ){
				    if( ! empty( $value['mwb-type'] ) && 'toggle-element-fixed' == $value['mwb-type'] && isset( $value['save_single_options'] ) && true === $value['save_single_options'] ){
					    $mwb_options[ $key ] = array_merge( $mwb_options[ $key ] , $value['elements'] );
				    }
			    }
		    }
		    return $mwb_options;
