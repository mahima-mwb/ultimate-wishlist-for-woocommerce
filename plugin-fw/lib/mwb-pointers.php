    ...
    class mwb_Pointers {
        ...
        public function __construct() {
            ...
            $this->_default_pointer['plugins'] = array(
                'screen_id'  => 'plugins',
                'options'    => array(
                    'content'  => sprintf( '<h3> %s </h3> <p> %s </p> <p> %s <a href="https://mwbemes.com/product-category/plugins/" target="_blank">mwbemes.com</a> %s
                                  <a href="https://profiles.wordpress.org/mwbemes/" target="_blank">Wordpress.org</a></p>',
                                __( 'Plugins Activated', 'mwb-plugin-fw' ),
                                __( 'From now on, you can find all plugin options in mwb Plugins menu.
                                     Plugin customization settings will be available as a new entry in mwb Plugins menu.', 'mwb-plugin-fw' ),
                                __( 'Discover all our plugins available on:', 'mwb-plugin-fw' ),
                                __( 'and', 'mwb-plugin-fw' )
                    ),
                ),
            );
            ...
            $this->_default_pointer['update'] = array(
                ...
                'options'    => array(
                    'content'  => sprintf( '<h3> %s </h3> <p> %s </p> <p> %s <a href="https://mwbemes.com/product-category/plugins/" target="_blank">mwbemes.com</a> %s
                                  <a href="https://profiles.wordpress.org/mwbemes/" target="_blank">Wordpress.org</a></p>',
                                __( 'Plugins Upgraded', 'mwb-plugin-fw' ),
                                __( 'From now on, you can find the option panel of mwb plugins in mwb Plugins menu.
                                    Every time one of our plugins is added, a new entry will be added to this menu.
                                    For example, after the update, plugin options (such as for mwb WooCommerce Wishlist, mwb WooCommerce Ajax Search, etc.)
                                    will be moved from previous location to mwb Plugins tab.', 'mwb-plugin-fw' ),
                                __( 'Discover all our plugins available on:', 'mwb-plugin-fw' ),
                                __( 'and', 'mwb-plugin-fw' )
                    ),
                ),
             ...
             */
            $this->special_screen = apply_filters( 'mwb-pointer-special-screen', array( 'plugins', 'update' ) );
        ...
        public function parse_args( $args ) {
            $default = array(
                'pointer_id' => 'mwb_default_pointer',
                'target'     => '#toplevel_page_mwb_plugin_panel',
                'init'       => null
            );
        ...
        public function add_pointers(){
            if( ! empty( $this->screen_ids ) ){
                foreach( $this->screen_ids as $screen_id ){
                    add_filter( "mwb_pointers-{$screen_id}", array( $this, 'pointers' ) );
                }
            }
        }
        ...
        public function pointer_load( $hook_suffix ) {
            ...
            $pointers   = apply_filters( "mwb_pointers-{$screen->id}", array() );
            ...
            if( in_array( $screen->id, $this->special_screen ) ){
                ...
                $recently_activate  = get_option( 'mwb_recently_activated', array() );
                ...
                if( $is_single_activate || $is_single_upgrade ){
                    ...
                    if( $is_multi_upgrade && isset( $_POST['checked'] ) && ( count( $_POST['checked'] )  > 0 ) ){
                        ...
                        $screen_id          = 'update';
                        $pointers           = apply_filters( "mwb_pointers-{$screen_id}", array() );
                    }
                     ...
                     *
                     * count( $point_id ) is the number of mwb plugins that have registered specific pointers
                ...
                update_option( 'mwb_recently_activated', array() );
            ...
            $script_file = function_exists( 'mwb_load_js_file' ) ? mwb_load_js_file( 'mwb-wp-pointer.js' ) : 'mwb-wp-pointer.min.js';
            ...
            wp_enqueue_script( 'mwb-wp-pointer', mwb_CORE_PLUGIN_URL . '/assets/js/' . $script_file, array( 'wp-pointer' ), false, true );
            wp_localize_script( 'mwb-wp-pointer', 'custom_pointer', $valid_pointers );
        }

        public function pointers( $pointers ){
            $screen_id          = str_replace( 'mwb_pointers-', '', current_filter() );
...
if ( ! function_exists( 'mwb_Pointers' ) ) {
    /**
     * Main instance of plugin
     *
     * @return object mwb_Pointers
     * @since  1.0
     * @author Andrea Grillo <andrea.grillo@mwbemes.com>
     */
    function mwb_Pointers() {
        return mwb_Pointers::instance();
    }
}

/**
 * Instance a mwb_Pointers object
 */
mwb_Pointers();