...
if ( ! class_exists( 'mwb_Plugin_Panel' ) ) {
	/**
	 * mwb Plugin Panel
	...
	class mwb_Plugin_Panel {
		...
		public function __construct( $args = array() ) {
			...
			if ( ! empty( $args ) ) {
				...
				$default_args = array(
					...
					'page_title'  => __( 'Plugin Settings', 'mwb-plugin-fw' ),
					'menu_title'  => __( 'Settings', 'mwb-plugin-fw' ),
					'capability'  => 'manage_options',
					'icon_url'    => '',
					'position'    => null,
				);

				$args = apply_filters( 'mwb_plugin_fw_panel_option_args', wp_parse_args( $args, $default_args ) );
				if ( isset( $args['parent_page'] ) && 'mwb_plugin_panel' === $args['parent_page'] ) {
					$args['parent_page'] = 'mwb_plugin_panel';
				}
			...
			//mwb-plugin-ui
			add_action( 'mwb_plugin_fw_before_mwb_panel', array( $this, 'add_plugin_banner' ), 10, 1 );
			add_action( 'wp_ajax_mwb_plugin_fw_save_toggle_element', array( $this, 'save_toggle_element_options' ) );
		}
		 ...
		 *
		 * @return bool
		 * @see      mwb_Plugin_Panel::init_wp_with_tabs
		 * @since    3.4.17
		 */
		...
		/**
		 * Init actions to show mwb Panel tabs in WP Pages
		 *
		...
		public function init_wp_with_tabs() {
			...
			if ( $tabs ) {
				...
				if ( ! $is_block_editor ) {
					wp_enqueue_style( 'mwb-plugin-style' );
					wp_enqueue_style( 'mwb-plugin-fw-fields' );
					wp_enqueue_script( 'mwb-plugin-fw-wp-pages' );
				}
		...
		protected static function _init_actions() {
			if ( ! static::$_actions_initialized ) {
				...
				// sort plugins by name in mwb Plugins menu
				add_action( 'admin_menu', array( __CLASS__, 'sort_plugins' ), 90 );
				add_filter( 'add_menu_classes', array( __CLASS__, 'add_menu_class_in_mwb_plugin' ) );
		...
		protected function _maybe_init_vars() {
			if ( ! isset( $this->_main_array_options ) && ! isset( $this->_tabs_hierarchy ) ) {
				...
				foreach ( $this->settings['admin-tabs'] as $item => $v ) {
					...
					$path = apply_filters( 'mwb_plugin_panel_item_options_path', $path, $options_path, $item, $this );
					if ( file_exists( $path ) ) {
						$_tab                      = include $path;
						...
						foreach ( $sub_tabs as $sub_item => $sub_options ) {
							...
							$sub_tab_path = apply_filters( 'mwb_plugin_panel_sub_tab_item_options_path', $sub_tab_path, $sub_tabs, $sub_item, $this );
		...
		/**
		 * Add mwb-plugin-fw-panel in body classes in Panel pages
		 *
		...
		public static function add_body_class( $admin_body_classes ) {
			global $pagenow;
			if ( ( 'admin.php' == $pagenow && strpos( get_current_screen()->id, 'mwb-plugins_page' ) !== false ) ) {
				$admin_body_classes = substr_count( $admin_body_classes, ' mwb-plugin-fw-panel ' ) == 0 ? $admin_body_classes . ' mwb-plugin-fw-panel ' : $admin_body_classes;
			}
		...
		public function add_menu_page() {
			...
			if ( ! isset( $admin_page_hooks['mwb_plugin_panel'] ) ) {
				...
				$capability = apply_filters( 'mwb_plugin_panel_menu_page_capability', 'manage_options' );
				$show       = apply_filters( 'mwb_plugin_panel_menu_page_show', true );

				//  mwb text must not be translated
				if ( ! ! $show ) {
					add_menu_page( 'mwb_plugin_panel', 'mwb', $capability, 'mwb_plugin_panel', null, mwb_plugin_fw_get_default_logo(), $position );
		 ...
		 * Submenu page hack: Remove the duplicate mwb Plugin link on subpages
		 *
		 * @return void
		 * @since    1.0
		 * @author   Andrea Grillo <andrea.grillo@mwbemes.com>
		...
		public function admin_enqueue_scripts() {
			...
			if ( 'admin.php' === $pagenow && strpos( get_current_screen()->id, $this->settings['page'] ) !== false || apply_filters( 'mwb_plugin_panel_asset_loading', false ) ) {
				wp_enqueue_media();

				wp_enqueue_style( 'mwb-plugin-fw-fields' );
				wp_enqueue_style( 'mwb-jquery-ui-style' );
				wp_enqueue_style( 'raleway-font' );

				wp_enqueue_script( 'jquery-ui' );
				wp_enqueue_script( 'jquery-ui-core' );
				wp_enqueue_script( 'jquery-ui-dialog' );
				wp_enqueue_script( 'mwb_how_to' );
				wp_enqueue_script( 'mwb-plugin-fw-fields' );
			}

			if ( ( 'admin.php' == $pagenow && mwb_plugin_fw_is_panel() ) || apply_filters( 'mwb_plugin_panel_asset_loading', false ) ) {
				wp_enqueue_media();
				wp_enqueue_style( 'mwb-plugin-style' );
				wp_enqueue_script( 'mwb-plugin-panel' );
			}

			if ( 'admin.php' == $pagenow && strpos( get_current_screen()->id, 'mwb_upgrade_premium_version' ) !== false ) {
				wp_enqueue_style( 'mwb-upgrade-to-pro' );
		...
		public function register_settings() {
			register_setting( 'mwb_' . $this->settings['parent'] . '_options', 'mwb_' . $this->settings['parent'] . '_options', array( $this, 'options_validate' ) );
		}
		...
		public function options_validate( $input ) {
			...
			foreach ( $mwb_options[ $option_key ] as $section => $data ) {
					...
					if ( isset( $option['sanitize_call'] ) && isset( $option['id'] ) ) { //yiw_debug($option, false);
						...
						if ( isset( $option['id'] ) ) {
							...
							if ( isset( $option['type'] ) && in_array( $option['type'], array( 'checkbox', 'onoff' ) ) ) {
								$value = mwb_plugin_fw_is_true( $value ) ? 'yes' : 'no';
							}

							if ( ! empty( $option['mwb-sanitize-callback'] ) && is_callable( $option['mwb-sanitize-callback'] ) ) {
								$value = call_user_func( $option['mwb-sanitize-callback'], $value );
							}
		...
		public function add_setting_page() {
			...
			if ( ! empty( $parent ) ) {
				add_submenu_page( $parent, $this->settings['page_title'], $this->settings['menu_title'], $this->settings['capability'], $this->settings['page'], array( $this, 'mwb_panel' ) );
			} else {
				add_menu_page( $this->settings['page_title'], $this->settings['menu_title'], $this->settings['capability'], $this->settings['page'], array( $this, 'mwb_panel' ), $this->settings['icon_url'], $this->settings['position'] );
			}
			/* === Duplicate Items Hack === */
			$this->remove_duplicate_submenu_page();
			do_action( 'mwb_after_add_settings_page' );
		...
		public function add_premium_version_upgrade_to_menu() {
			...
			if ( function_exists( 'mwb_Plugin_Licence' ) && ! ! mwb_Plugin_Licence()->get_products() ) {
				return;
			}

			global $submenu;
			if ( apply_filters( 'mwb_show_upgrade_to_premium_version', isset( $submenu['mwb_plugin_panel'] ) ) ) {
				$submenu['mwb_plugin_panel']['how_to'] = array(
					sprintf( '%s%s%s', '<span id="mwb-how-to-premium">', __( 'How to install premium version', 'mwb-plugin-fw' ), '</span>' ),
					'install_plugins',
					'//support.mwbemes.com/hc/en-us/articles/217840988',
					__( 'How to install premium version', 'mwb-plugin-fw' ),
				);
			}
		}
		...
		public function print_tabs_nav( $args = array() ) {
			$defaults = array(
				...
				'premium_class' => isset( $this->settings['class'] ) ? 'mwb-premium' : 'premium',
				'page'          => $this->settings['page'],
			...
			$tabs = '<ul class="mwb-plugin-fw-tabs">';
			...
			foreach ( $this->settings['admin-tabs'] as $tab => $tab_value ) {
				...
				$active_class = apply_filters( 'mwb_plugin_fw_panel_active_tab_class', $active_class, $current_tab, $tab );
				...
				$icon          = ( $current_tab !== $tab && $sub_tabs ) ? '<i class="mwb-icon mwb-icon-arrow_down"></i>' : '';

				$tabs .= '<li class="mwb-plugin-fw-tab-element">';
		...
		public function get_nav_url( $page, $tab, $sub_tab = '', $parent_page = '' ) {
			...
			return apply_filters( 'mwb_plugin_fw_panel_url', $url, $page, $tab, $sub_tab, $parent_page );
		}
		...
		public function print_sub_tabs_nav( $args = array() ) {
			...
			if ( $sub_tabs && $current_sub_tab ) {
				include mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/sub-tabs-nav.php';
			}
		}
		...
		public function mwb_panel() {
			...
			$custom_tab_action = $this->is_custom_tab( $mwb_options, $option_key );
			?>
			<div class="wrap <?php echo $wrap_class ?>">
				<div id="icon-themes" class="icon32"><br/></div>
				<?php
				do_action( 'mwb_plugin_fw_before_mwb_panel', $this->settings['page'] );
						...
						<?php if ( $this->is_show_form() ) : ?>
							<form id="mwb-plugin-fw-panel" method="post" action="options.php">
								...
								<input type="submit" class="button-primary"
										value="<?php _e( 'Save Changes', 'mwb-plugin-fw' ) ?>"
							...
							<form method="post">
								...
								<input type="submit" name="mwb-reset" class="button-secondary"
										value="<?php _e( 'Reset to default', 'mwb-plugin-fw' ) ?>"
		...
		public function add_fields() {
			...
			foreach ( $mwb_options[ $option_key ] as $section => $data ) {
				add_settings_section( "mwb_settings_{$option_key}_{$section}", $this->get_section_title( $section ), $this->get_section_description( $section ), 'mwb' );
				foreach ( $data as $option ) {
					if ( isset( $option['id'] ) && isset( $option['type'] ) && isset( $option['name'] ) ) {
						add_settings_field( "mwb_setting_" . $option['id'],  $option['name'], array( $this, 'render_field' ), 'mwb', "mwb_settings_{$option_key}_{$section}", array( 'option' => $option, 'label_for' => $this->get_id_field( $option['id'] ) ) );
					}
				}
			}
		}
		...
		function get_current_tab() {
			...
			if ( isset( $_REQUEST['mwb_tab_options'] ) ) {
				return $_REQUEST['mwb_tab_options'];
		...
		public function message() {
			...
			$message = array(
				'element_exists'   => $this->get_message( '<strong>' . __( 'The element you have entered already exists. Please, enter another name.', 'mwb-plugin-fw' ) . '</strong>', 'error', false ),
				'saved'            => $this->get_message( '<strong>' . __( 'Settings saved', 'mwb-plugin-fw' ) . '.</strong>', 'updated', false ),
				'reset'            => $this->get_message( '<strong>' . __( 'Settings reset', 'mwb-plugin-fw' ) . '.</strong>', 'updated', false ),
				'delete'           => $this->get_message( '<strong>' . __( 'Element deleted correctly.', 'mwb-plugin-fw' ) . '</strong>', 'updated', false ),
				'updated'          => $this->get_message( '<strong>' . __( 'Element updated correctly.', 'mwb-plugin-fw' ) . '</strong>', 'updated', false ),
				'settings-updated' => $this->get_message( '<strong>' . __( 'Element updated correctly.', 'mwb-plugin-fw' ) . '</strong>', 'updated', false ),
				'imported'         => $this->get_message( '<strong>' . __( 'Database imported correctly.', 'mwb-plugin-fw' ) . '</strong>', 'updated', false ),
				'no-imported'      => $this->get_message( '<strong>' . __( 'An error has occurred during import. Please try again.', 'mwb-plugin-fw' ) . '</strong>', 'error', false ),
				'file-not-valid'   => $this->get_message( '<strong>' . __( 'The added file is not valid.', 'mwb-plugin-fw' ) . '</strong>', 'error', false ),
				'cant-import'      => $this->get_message( '<strong>' . __( 'Sorry, import is disabled.', 'mwb-plugin-fw' ) . '</strong>', 'error', false ),
				'ord'              => $this->get_message( '<strong>' . __( 'Sorting successful.', 'mwb-plugin-fw' ) . '</strong>', 'updated', false ),
			);
		...
		public function get_default_options() {
			...
			foreach ( $mwb_options as $tab => $sections ) {
		...
		function get_tab_title() {
			...
			foreach ( $mwb_options[ $option_key ] as $sections => $data ) {
				foreach ( $data as $option ) {
		...
		function get_section_title( $section ) {
			...
			foreach ( $mwb_options[ $option_key ][ $section ] as $option ) {
		...
		function get_name_field( $name = '' ) {
			return 'mwb_' . $this->settings['parent'] . '_options[' . $name . ']';
		}
		...
		function get_id_field( $id ) {
			return 'mwb_' . $this->settings['parent'] . '_options_' . $id;
		}
		...
		function render_field( $param ) {
			...
			if ( ! empty( $param ) && isset( $param ['option'] ) ) {
				...
				if ( $field_template_path = mwb_plugin_fw_get_field_template_path( $option ) ) {
					$field_container_path = apply_filters( 'mwb_plugin_fw_panel_field_container_template_path', mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/panel-field-container.php', $option );
					file_exists( $field_container_path ) && include( $field_container_path );
				} else {
					do_action( "mwb_panel_{$option['type']}", $option, $db_value, $custom_attributes );
				}
			}
		}
		...
		public function get_options() {
			$options = get_option( 'mwb_' . $this->settings['parent'] . '_options' );
			if ( $options === false || ( isset( $_REQUEST['mwb-action'] ) && $_REQUEST['mwb-action'] == 'reset' ) ) {
		...
		public static function add_infobox( $args = array() ) {
			if ( ! empty( $args ) ) {
				extract( $args );
				require_once( mwb_CORE_PLUGIN_TEMPLATE_PATH . '/panel/boxinfo.php' );
			}
		}
		...
		public static function sort_plugins() {
			global $submenu;
			if ( ! empty( $submenu['mwb_plugin_panel'] ) ) {
				$sorted_plugins = $submenu['mwb_plugin_panel'];

				usort( $sorted_plugins, function ( $a, $b ) {
					return strcmp( current( $a ), current( $b ) );
				} );

				$submenu['mwb_plugin_panel'] = $sorted_plugins;
			}
		}

		/**
		 * add menu class in mwb Plugins menu
		 *
		 * @since    3.0.0
		 * @author   Leanza Francesco <leanzafrancesco@gmail.com>
		 */
		public static function add_menu_class_in_mwb_plugin( $menu ) {
			global $submenu;

			if ( ! empty( $submenu['mwb_plugin_panel'] ) ) {
				$item_count = count( $submenu['mwb_plugin_panel'] );
				$columns    = absint( $item_count / 20 ) + 1;
				$columns    = max( 1, min( $columns, 3 ) );
				$columns    = apply_filters( 'mwb_plugin_fw_mwb_plugins_menu_columns', $columns, $item_count );

				if ( $columns > 1 ) {
					$class = "mwb-plugin-fw-menu-$columns-columns";
					foreach ( $menu as $order => $top ) {
						if ( 'mwb_plugin_panel' === $top[2] ) {
							$c                 = $menu[ $order ][4];
		...
		public function add_plugin_banner( $page ) {
			...
			if ( $this->is_free() && isset( $this->settings['plugin_slug'] ) ):
				$rate_link = apply_filters( 'mwb_plugin_fw_rate_url', 'https://wordpress.org/support/plugin/' . $this->settings['plugin_slug'] . '/reviews/?rate=5#new-post' );
				?>
				<h1 class="notice-container"></h1>
				<div class="mwb-plugin-fw-banner">
					<h1><?php echo esc_html( $this->settings['page_title'] ) ?></h1>
				</div>
				<div class="mwb-plugin-fw-rate">
					<?php printf( '<strong>%s</strong> %s <a href="%s" target="_blank"><u>%s</u> <span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span></a>  %s',
								  __( 'We need your support', 'mwb-plugin-fw' ),
								  __( 'to keep updating and improving the plugin. Please,', 'mwb-plugin-fw' ),
								  $rate_link,
								  __( 'help us by leaving a five-star rating', 'mwb-plugin-fw' ),
								  __( ':) Thanks!', 'mwb-plugin-fw' ) ) ?>
				</div>
			<?php else: ?>
				<h1 class="notice-container"></h1>
				<div class="mwb-plugin-fw-banner">
		 ...
		 * @author Emanuela Castorina
		 */
		public function add_mwb_ui( $field ) {
			global $pagenow;
			...
			if ( 'admin.php' === $pagenow && $screen && strpos( $screen->id, $this->settings['page'] ) !== false ) {
				switch ( $field['type'] ) {
					case 'datepicker':
						echo '<span class="mwb-icon mwb-icon-calendar"></span>';
						break;
					default:
						break;
				}
		...
		public function get_post_type_tabs( $post_type ) {
			...
			return apply_filters( "mwb_plugin_fw_panel_{$panel_page}_get_post_type_tabs", $tabs, $post_type );
		}
		...
		public function get_taxonomy_tabs( $taxonomy ) {
			...
			return apply_filters( "mwb_plugin_fw_panel_{$panel_page}_get_taxonomy_tabs", $tabs, $taxonomy );
		}
		...
		public function maybe_redirect_to_proper_wp_page() {
			...
			if ( 'admin.php' === $pagenow && isset( $_GET['page'] ) && $this->settings['page'] === $_GET['page'] &&
				 ! $this->is_custom_taxonomy_page() && ! $this->is_custom_post_type_page() && ! isset( $_REQUEST['mwb-plugin-fw-panel-skip-redirect'] ) ) {
				...
				if ( strpos( $url, 'edit.php' ) !== false || strpos( $url, 'edit-tags.php' ) !== false ) {
					wp_safe_redirect( add_query_arg( array( 'mwb-plugin-fw-panel-skip-redirect' => 1 ), $url ) );
					exit;
				}
			}
		}
		 ...
		 * Important: this opens a wrapper <div> that will be closed through mwb_Plugin_Panel::print_panel_tabs_in_post_edit_page_end()
		 *
		 * @since    3.4.0
		 * @author   Leanza Francesco <leanzafrancesco@gmail.com>
		 */
		public function print_panel_tabs_in_wp_pages() {
			if ( self::$_panel_tabs_in_wp_pages ) {
				...
				?>
				<div class="mwb-plugin-fw-wp-page-wrapper">
				<?php
		...
		/**
		 * Close the wrapper opened in mwb_Plugin_Panel::print_panel_tabs_in_wp_pages()
		 *
		 * @since    3.4.0
		 * @author   Leanza Francesco <leanzafrancesco@gmail.com>
		 */
