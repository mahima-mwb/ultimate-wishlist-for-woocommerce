...
class mwb_CPT_Unlimited {
     ...
     *
     * @since 1.0
     * @author Antonino Scarfi' <antonino.scarfi@mwbemes.com>
     */
    public function __construct( $args = array() ) {
        ...
        if ( $this->_args['add_multiuploader'] ) {
            ...
            add_action( 'wp_ajax_mwb_cptu_multiuploader', array( $this, 'post_multiuploader' ) );
        }
        ...
        if ( $this->_args['has_single'] ) {
            add_action( 'mwb_loop', array( $this, 'single_template' ) );
        ...
        if ( ! empty( $this->_args['shortcode_name'] ) ) {
            ...
            add_filter( 'mwb_shortcode_' . $this->_args['shortcode_name'] . '_icon', array( $this, 'shortcode_icon') );
            add_filter( 'mwb-shortcode-plugin-init', array( $this, 'add_shortcode_to_box' ) );
        }
	...
	public function admin_sortable_assets() {
		...
		wp_enqueue_script( 'mwb-cptu-sortable-posts', mwb_CORE_PLUGIN_URL . '/assets/js/mwb-cptu-sortable-posts.js', array( 'jquery', 'jquery-ui-sortable' ), '1.0', true );
	}
    ...
    public function register_post_type() {
        $labels = array(
            ...
            'add_new'            => sprintf( __( 'Add %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'add_new_item'       => sprintf( __( 'Add New %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'edit_item'          => sprintf( __( 'Edit %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'new_item'           => sprintf( __( 'New %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'all_items'          => sprintf( __( 'All %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'view_item'          => sprintf( __( 'View %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'search_items'       => sprintf( __( 'Search %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found'          => sprintf( __( 'No %s found', 'mwb-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'mwb-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'parent_item_colon'  => '',
            'menu_name'          => ucfirst( $this->_labels['menu'] )
        );

        $args = array(
            'labels'             => apply_filters( 'mwb_' . $this->_name . '_labels', $labels ),
            'public'             => false,
        ...
        register_post_type( $this->_name, apply_filters( 'mwb_' . $this->_name . '_args', $args ) );
    }
    ...
    protected function _cpt_args( $post ) {
        ...
        $args = apply_filters( 'mwb_cptu_register_post_type_args', array(
    ...
    public function register_cptu_post_types() {
        ...
        foreach ( $post_types as $pt ) {
            ...
            $labels = array(
                ...
                'add_new'            => sprintf( __( 'Add %s', 'mwb-plugin-fw' ), $label_singular ),
                'add_new_item'       => sprintf( __( 'Add New %s', 'mwb-plugin-fw' ), $label_singular ),
                'edit_item'          => sprintf( __( 'Edit %s', 'mwb-plugin-fw' ), $label_singular ),
                'new_item'           => sprintf( __( 'New %s', 'mwb-plugin-fw' ), $label_singular ),
                'all_items'          => sprintf( __( 'All %s', 'mwb-plugin-fw' ), $label_plural ),
                'view_item'          => sprintf( __( 'View %s', 'mwb-plugin-fw' ), $label_singular ),
                'search_items'       => sprintf( __( 'Search %s', 'mwb-plugin-fw' ), $label_plural ),
                'not_found'          => sprintf( __( 'No %s found', 'mwb-plugin-fw' ), $label_plural ),
                'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'mwb-plugin-fw' ), $label_plural ),
                'parent_item_colon'  => '',
                'menu_name'          => $title
            );

            $args = array(
                'labels'             => apply_filters( 'mwb_' . $this->_prefix_cpt . $name . '_labels', $labels ),
                'public'             => true,
            ...
            // register post type
            $post_type = mwb_avoid_duplicate( str_replace( '-', '_', substr( $this->_prefix_cpt . $name, 0, 16) ), $post_types );
            register_post_type( $post_type, apply_filters( 'mwb_' . $this->_prefix_cpt . $name . '_args', $args, $pt ) );  // save the post type in post meta
            ...
            if ( $this->_args['has_taxonomy'] && ! empty( $taxonomy ) ) {
                ...
                $labels = array(
                    'name'              => sprintf( _x( '%s Categories', 'taxonomy general name', 'mwb-plugin-fw' ), $label_singular ),
                    'singular_name'     => _x( 'Category', 'taxonomy singular name', 'mwb-plugin-fw' ),
                    'search_items'      => __( 'Search Categories', 'mwb-plugin-fw' ),
                    'all_items'         => __( 'All Categories', 'mwb-plugin-fw' ),
                    'parent_item'       => __( 'Parent Category', 'mwb-plugin-fw' ),
                    'parent_item_colon' => __( 'Parent Category:', 'mwb-plugin-fw' ),
                    'edit_item'         => __( 'Edit Category', 'mwb-plugin-fw' ),
                    'update_item'       => __( 'Update Category', 'mwb-plugin-fw' ),
                    'add_new_item'      => __( 'Add New Category', 'mwb-plugin-fw' ),
                    'new_item_name'     => __( 'New Category Name', 'mwb-plugin-fw' ),
                    'menu_name'         => __( 'Category', 'mwb-plugin-fw' ),
                );
        ...
        wp_cache_set( 'mwb_cptu_post_types', $post_types );
    }
    ...
    public function add_cptu_menu_item() {
        ...
        if ( strpos( $post_type, $this->_prefix_cpt ) !== false ) {
            ...
            $submenu[ $item ][20] = array( sprintf( __('Add %s', 'mwb-plugin-fw'), ucfirst( $label_singular ) ), 'edit_posts', 'post-new.php?post_type=' . $post_type );

            global $wp_taxonomies;
            $taxonomy = get_post_meta( $portfolio->ID, '_taxonomy', true );
            if ( isset( $wp_taxonomies[ $taxonomy ] ) ) {
                $submenu[ $item ][25] = array( __('Categories', 'mwb-plugin-fw'), 'edit_posts', 'edit-tags.php?taxonomy=' . $taxonomy . '&post_type=' . $post_type );
            }
        }
    ...
    public function frontend_assets() {
        ...
        if ( ! is_single() || ! isset( $post->post_type ) || ! $this->_is_valid( $post->post_type ) ) {
            ...
            foreach ( $posts as $post_id ) {
                ...
                foreach ( $this->layouts[ $layout ]['css'] as $asset ) {
                    ...
                    mwb_enqueue_style( $asset['handle'], empty( $asset['src'] ) ? false : $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['media'] );
                }
        ...
        // load assets of single template
            ...
            foreach ( $this->layouts[ $layout ]['css'] as $asset ) {
                ...
                mwb_enqueue_style( $asset['handle'], $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['media'] );
            }
        }
    ...
    public function add_metabox_cptu() {
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            $layouts = apply_filters( 'mwb_cptu_' . $this->_name . '_layout_values', $layouts );
        }

        $single_layouts = apply_filters( 'mwb_cptu_' . $this->_name . '_single_layout_values', array() );

        $metabox_args = array(
            'label'    => sprintf( __( '%s Settings', 'mwb-plugin-fw' ), $this->_labels['singular'] ),
            'pages'    => $this->_name, //or array( 'post-type1', 'post-type2')
            'context'  => 'normal', //('normal', 'advanced', or 'side')
            'priority' => 'default',
            'tabs'     => array(
                'settings' => array(
                    'label'  => __( 'Settings', 'mwb-plugin-fw' ),
                    'fields' => apply_filters( 'mwb_cptu_fields', array(
                        'type'      => array(
                            'label' => __( 'Type', 'mwb-plugin-fw' ),
                            'desc'  => sprintf( __( 'Layout for this %s' , 'mwb-plugin-fw' ), strtolower( $this->_labels['singular'] ) ),
                            'type'  => 'select',
                            'options' => isset( $layouts ) ? $layouts : array(),
                            'std'   => '' ),

                        'rewrite'     => array(
                            'label' => __( 'Rewrite', 'mwb-plugin-fw' ),
                            'desc'  => __( 'Univocal identification name in the URL for each product (slug from post if empty)', 'mwb-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_singular' => array(
                            'label' => __( 'Label in Singular', 'mwb-plugin-fw' ),
                            'desc'  => __( 'Set a label in singular (title of portfolio if empty)', 'mwb-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_plural' => array(
                            'label' => __( 'Label in Plural', 'mwb-plugin-fw' ),
                            'desc'  => __( 'Set a label in plural (title of portfolio if empty)', 'mwb-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy' => array(
                            'label' => __( 'Taxonomy', 'mwb-plugin-fw' ),
                            'desc'  => __( 'If you want to use categories in the portfolio, set a name for taxonomy. Name should be a slug (must not contain capital letters nor spaces) and must not be more than 32 characters long (database structure restriction).', 'mwb-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy_rewrite' => array(
                            'label' => __( 'Taxonomy Rewrite', 'mwb-plugin-fw' ),
                            'desc'  => __( 'Set univocal name for each category page URL.', 'mwb-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'single_layout' => array(
                            'label' => __( 'Single layout', 'mwb-plugin-fw' ),
                            'desc'  => __( 'Layout for single page of this portfolio', 'mwb-plugin-fw' ),
                            'type'  => 'select',
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            foreach ( $this->layouts as $layout => $args ) {
                ...
                $metabox_args['tabs']['settings']['fields'][ $layout . '_title' ] = array(
                    'desc' => $args['name'] . ' ' . __( 'layout settings', 'mwb-plugin-fw' ),
                    'type' => 'title',
                    'deps' => array(
                        'ids' => '_type',
                        'values' => $layout
        ...
        $metabox = mwb_Metabox( $this->_name . '_cptu_settings' );
    ...
    public function add_metabox_item_fields() {
        ...
        $metabox = mwb_Metabox( $post_type . '_item_fields' );
        $metabox->init( $metabox_args );
    ...
    public function add_quick_links_metaboxes() {
        // CPTU
        add_meta_box( $this->_name . '_quick_links', __( 'Quick links', 'mwb-plugin-fw' ), array( $this, 'quick_links_cptu_inner' ), $this->_name, 'side', 'high' );
        ...
        foreach ( $post_types as $post ) {
            ...
            add_meta_box( $post->post_type . '_quick_links', __( 'Quick links', 'mwb-plugin-fw' ), array( $this, 'quick_links_cpt_inner' ), $post_type, 'side', 'high' );
        }
    }
    ...
    public function quick_links_cptu_inner( $post ) {
        extract( $this->_cpt_args( $post ) );
        ?>
        <a href="<?php echo admin_url( 'edit.php?post_type=' . get_post_meta( $post->ID, '_post_type', true ) ) ?>"><?php printf( __( 'View %s', 'mwb-plugin-fw' ), $label_plural ) ?></a>
        <?php
    }
    ...
    public function quick_links_cpt_inner( $post ) {
        ...
        ?>
        <a href="<?php echo admin_url( "post.php?post={$post->ID}&action=edit" ) ?>"><?php printf( __( 'Edit %s', 'mwb-plugin-fw' ), $post->post_title ) ?></a>
        <?php
    }
    ...
    public function cptu_change_columns( $column, $post_id ) {
        ...
        switch ( $column ) {
            case 'actions' :
                echo '<a href="' . admin_url( "post.php?post={$post_id}&action=edit" ) . '" class="button-secondary">' . sprintf( __( 'Edit %s', 'mwb-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ) . '</a> ';
                echo '<a href="' . admin_url( 'edit.php?post_type=' . get_post_meta( $post_id, '_post_type', true ) ) . '" class="button-secondary">' . sprintf( __( 'View %s', 'mwb-plugin-fw' ), $label_plural ) . '</a> ';
                break;
        }
    }
    ...
    public function add_shortcode( $atts, $content = null ) {
        ...
        if( function_exists( 'mwb_Portfolio' ) && method_exists( mwb_Portfolio(), 'is' ) && mwb_Portfolio()->is( $atts['name'] ) ){
            $atts['name'] = str_replace( mwb_Portfolio()->post_type_prefix, '', $atts['name'] );
        }
        ...
        extract( apply_filters( 'mwb_cptu_frontend_vars', $atts, $this->_name ) );
        ...
        if ( $this->_args['manage_layouts'] && isset( $this->layouts[ $layout ]['js'] ) && ! empty( $this->layouts[ $layout ]['js'] ) ) {
            foreach ( $this->layouts[ $layout ]['js'] as $asset ) {
                ...
                mwb_enqueue_script( $asset['handle'], $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['in_footer'] );
            }
        }
     ...
     *
     * @param $icon_url string Icone url found by mwb_shortcode plugin
     *
     * @return string
     * @since 1.0.0
     * @author Antonino Scarfi' <antonio.scarfi@mwbemes.it>
     */
     ...
     *
     * All definition settings to add cptu shortcode to mwb Shortcode Panel
     *
    ...
    public function add_shortcode_to_box( $shortcodes ) {
        ...
        $args = array(
            $this->_args['shortcode_name'] => array(
                ...
                'description' => sprintf( __( 'Show frontend of the %s', 'mwb-plugin-fw' ), $this->_labels['main_name'] ),
                'tab'         => 'cpt',
                'create'      => false,
                'has_content' => false,
    ...
    */
    ...
    public function admin_assets() {
        ...
        wp_enqueue_media();
        wp_enqueue_script( 'mwb-cptu', mwb_CORE_PLUGIN_URL . '/assets/js/mwb-cpt-unlimited' . $suffix . '.js', array('jquery'), '', true );
    }
        ...
        <script>
            (function($) {
                ...
                var button = $('<a />', {
                    ...
                    'data-uploader_title': '<?php printf( __( 'Add %s from images', 'mwb-plugin-fw' ), $label_plural ) ?>',
                    'data-uploader_button_text': '<?php printf( __( 'Add %s', 'mwb-plugin-fw' ), $label_plural ) ?>',
                    'data-nonce': '<?php echo wp_create_nonce( 'cpt-unlimited-multiuploader' ); ?>'
                }).text('<?php _e( 'Upload multiple files', 'mwb-plugin-fw' ) ?>');
...
//        }
        ...
        if ( $this->_args['manage_layouts'] ) {
            foreach ( $this->layouts[ 'single' ]['js'] as $asset ) {
                mwb_enqueue_script( $asset['handle'], $this->locate_url( 'single' ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['in_footer'] );
            }
        }
        ...
        extract( apply_filters( 'mwb_cptu_frontend_vars', array( 'name' => $post_name ), $this->_name ) );
    ...
    public function archive_template() {
        ...
        remove_action( 'mwb_content_loop', 'mwb_content_loop', 10 );
        add_action( 'mwb_content_loop', array( $this, 'archive_template_loop' ), 10 );
