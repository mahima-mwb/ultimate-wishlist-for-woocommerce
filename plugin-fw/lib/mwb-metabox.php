...
if ( !class_exists( 'mwb_Metabox' ) ) {
    /**
     * mwb Metabox
     * the metabox can be created using this code
     * <code>
     * $args1 = array(
     *      'label'    => __( 'Metabox Label', 'mwb-plugin-fw' ),
     ...
     *                          'fields' => array(
     *                          'meta_checkbox' => array(
     *                                 'label'    => __( 'Show title', 'mwb-plugin-fw' ),
     *                                 'desc'     => __( 'Choose whether to show title of the page or not.', 'mwb-plugin-fw' ),
     *                                 'type'     => 'checkbox',
     *                                 'private'  => false,
     *                                 'std'      => '1'),
     *                            ),
     *                      ),
     *  );
     * $metabox1 = mwb_Metabox( 'mwb-metabox-id' );
     * $metabox1->init( $args );
     * </code>
     *
     * @class      mwb_Metaboxes
    ...
    class mwb_Metabox {
        ...
        public function init( $options = array() ) {
            ...
            add_filter( 'mwb_icons_screen_ids', array( $this, 'add_screen_ids_for_icons' ) );

            add_action( 'wp_ajax_mwb_plugin_fw_save_toggle_element_metabox', array( $this, 'save_toggle_element' ) );
        }
        ...
        public function enqueue() {
            ...
            $enqueue = apply_filters( 'mwb_plugin_fw_metabox_enqueue_styles_and_scripts', $enqueue, $this );
            ...
            if ( $enqueue ) {
                ...
                wp_enqueue_style( 'mwb-plugin-metaboxes' );
                wp_enqueue_style( 'mwb-jquery-ui-style' );

                wp_enqueue_script( 'mwb-metabox' );
        ...
        public function get_option_metabox_name( $id_field, $private = true ) {
            $db_name = apply_filters( 'mwb_metaboxes_option_main_name', 'mwb_metaboxes' );
            $return  = $db_name . '[';
        ...
        public function show() {
            ...
            mwb_plugin_get_template( mwb_CORE_PLUGIN_PATH, 'metaboxes/tab.php', $args );
        }
        ...
        public function save_postdata( $post_id ) {
            if ( !isset( $_POST[ 'mwb_metaboxes_nonce' ] ) || !wp_verify_nonce( $_POST[ 'mwb_metaboxes_nonce' ], 'metaboxes-fields-nonce' ) ) {
                return $post_id;
            }

            $allow_ajax = isset( $_REQUEST[ 'mwb_metabox_allow_ajax_saving' ] ) && $this->id === $_REQUEST[ 'mwb_metabox_allow_ajax_saving' ];
            ...
            if ( isset( $_POST[ 'mwb_metaboxes' ] ) ) {
                $mwb_metabox_data = $_POST[ 'mwb_metaboxes' ];

                if ( is_array( $mwb_metabox_data ) ) {
                    foreach ( $mwb_metabox_data as $field_name => $field_value ) {
        ...
        public function sanitize_and_save_fields( $post_id ) {
            ...
            $allow_ajax              = isset( $_REQUEST[ 'mwb_metabox_allow_ajax_saving' ] ) && $this->id === $_REQUEST[ 'mwb_metabox_allow_ajax_saving' ];
            $ajax_partial_saving_tab = isset( $_REQUEST[ 'mwb_metabox_allow_ajax_partial_saving_tab' ] ) ? $_REQUEST[ 'mwb_metabox_allow_ajax_partial_saving_tab' ] : false;
        ...
        public function sanitize_and_save_field( $field, $post_id ) {
            ...
            if ( isset( $_POST[ 'mwb_metaboxes' ][ $field[ 'id' ] ] ) ) {
                if ( in_array( $field[ 'type' ], array( 'onoff', 'checkbox' ) ) ) {
                    ...
                    if ( isset( $field[ 'elements' ] ) && $field[ 'elements' ] ) {
                        $elements_value = $_POST[ 'mwb_metaboxes' ][ $field[ 'id' ] ];
                        if ( $elements_value ) {
                            ...
                            foreach ( $field[ 'elements' ] as $element ) {
                                foreach ( $elements_value as $key => $element_value ) {
                                    ...
                                    if ( !empty( $element[ 'mwb-sanitize-callback' ] ) && is_callable( $element[ 'mwb-sanitize-callback' ] ) ) {
                                        $elements_value[ $key ][ $element[ 'id' ] ] = call_user_func( $element[ 'mwb-sanitize-callback' ], $elements_value[ $key ][ $element[ 'id' ] ] );
                                    }
                                }
                            }
                        }
                    ...
                    $value = $_POST[ 'mwb_metaboxes' ][ $field[ 'id' ] ];
                    if ( !empty( $field[ 'mwb-sanitize-callback' ] ) && is_callable( $field[ 'mwb-sanitize-callback' ] ) ) {
                        $value = call_user_func( $field[ 'mwb-sanitize-callback' ], $value );
                    }
        ...
        public function save_toggle_element() {
            ...
            if ( !isset( $_REQUEST[ 'mwb_metaboxes_nonce' ] ) || !wp_verify_nonce( $_REQUEST[ 'mwb_metaboxes_nonce' ], 'metaboxes-fields-nonce' ) ) {
                return;
            }
            $post_id = $_REQUEST[ 'post_ID' ];

            if ( isset( $_REQUEST[ 'mwb_metaboxes' ], $_REQUEST[ 'toggle_id' ], $_REQUEST[ 'metabox_tab' ], $_REQUEST[ 'mwb_metaboxes' ][ $_REQUEST[ 'toggle_id' ] ] ) ) {
                $mwb_metabox_data = $_REQUEST[ 'mwb_metaboxes' ];
...
if ( !function_exists( 'mwb_Metabox' ) ) {
    ...
    function mwb_Metabox( $id ) {
        return mwb_Metabox::instance( $id );
    }
}
