...
if ( !class_exists( 'mwb_Debug' ) ) {
    ...
    class mwb_Debug {
        ...
        public function init() {
            ...
            $is_debug = apply_filters( 'mwb_plugin_fw_is_debug', isset( $_GET[ 'mwb-debug' ] ) );
        ...
        public function add_debug_in_admin_bar( $wp_admin_bar ) {
            $args = array(
                'id'    => 'mwb-debug-admin-bar',
                'title' => 'mwb Debug',
                'href'  => '',
                'meta'  => array(
                    'class' => 'mwb-debug-admin-bar'
                )
            );
            ...
            foreach ( $this->get_debug_information() as $key => $information ) {
                ...
                $subnodes[] = array(
                    'id'     => 'mwb-debug-admin-bar-' . $key,
                    'parent' => 'mwb-debug-admin-bar',
                    'title'  => $title,
                    'href'   => $url,
                    'meta'   => array(
                        'class' => 'mwb-debug-admin-bar-' . $key
                    )
                );
                ...
                if ( isset( $information[ 'subsub' ] ) ) {
                    foreach ( $information[ 'subsub' ] as $sub_key => $sub_value ) {
                        ...
                        $subnodes[] = array(
                            'id'     => 'mwb-debug-admin-bar-' . $key . '-' . $sub_key,
                            'parent' => 'mwb-debug-admin-bar-' . $key,
                            'title'  => $title,
                            'href'   => '',
                            'meta'   => array(
                                'class' => 'mwb-debug-admin-bar-' . $key . '-' . $sub_key,
                                'html'  => $html,
                            )
                        );
                    }
        ...
        public function get_debug_information() {
            $debug = array(
                ...
                'mwb-premium-plugins' => array(
                    'label'  => 'mwb Premium Plugins',
                    'value'  => '',
                ...
                'post-meta'            => array(
                    'label' => 'Post Meta',
                    'value' => '',
                    'url'   => add_query_arg( array( 'mwb-debug-post-meta' => 'all' ) )
                ),
                'option'               => array(
                    'label' => 'Option',
                    'value' => '',
                    'url'   => add_query_arg( array( 'mwb-debug-option' => '' ) )
                ),
            );

            // Post Meta debug -------------
            global $post;
            if ( !empty( $_GET[ 'mwb-debug-post-meta' ] ) && $post ) {
                $meta_key   = $_GET[ 'mwb-debug-post-meta' ];
            ...
            if ( !empty( $_GET[ 'mwb-debug-option' ] ) ) {
                $option_key   = $_GET[ 'mwb-debug-option' ];
        ...
        public function get_plugin_framework_info() {
            $plugin_fw_version   = mwb_plugin_fw_get_version();
            $plugin_fw_loaded_by = basename( dirname( mwb_CORE_PLUGIN_PATH ) );
        ...
        public function get_premium_plugins_info() {
            $plugins      = mwb_Plugin_Licence()->get_products();
            $plugins_info = array();
...
if ( !function_exists( 'mwb_Debug' ) ) {
    function mwb_Debug() {
        return mwb_Debug::get_instance();
    }
