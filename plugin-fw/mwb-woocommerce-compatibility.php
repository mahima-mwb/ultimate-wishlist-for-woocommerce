...
if ( !function_exists( 'mwb_get_prop' ) ) {
    /**
     *
     */
    function mwb_get_prop( $object, $key, $single = true, $context = 'view' ) {

        $prop_map   = mwb_return_new_attribute_map();
        ...
        if ( $is_wc_data ) {
            ...
            if ( isset( $object->$key ) ) {
                return $object->$key;
            } elseif ( mwb_wc_check_post_columns( $key ) ) {
...
if ( !function_exists( 'mwb_set_prop' ) ) {
    /**
     *
     */
    function mwb_set_prop( $object, $arg1, $arg2 = false ) {
        ...
        foreach ( $arg1 as $key => $value ) {
            if ( $is_wc_data ) {
                ...
                if ( mwb_wc_check_post_columns( $key ) ) {
...
if ( !function_exists( 'mwb_save_prop' ) ) {
    /**
     *
     */
    function mwb_save_prop( $object, $arg1, $arg2 = false, $force_update = false ) {
        if ( !is_array( $arg1 ) ) {
            $arg1 = array(
                $arg1 => $arg2
            );
        ...
        foreach ( $arg1 as $key => $value ) {
            mwb_set_prop( $object, $key, $value );

            if ( !$is_wc_data ) {

                if ( mwb_wc_check_post_columns( $key ) ) {
                    mwb_store_changes( $object->post, $key, $value );
                } else {
                    $object_id = is_callable( array( $object, 'get_id' ) ) ? $object->get_id() : $object->id;
...
if ( !function_exists( 'mwb_delete_prop' ) ) {
    /**
     *
     */
    function mwb_delete_prop( $object, $key, $value = '' ) {
        $prop_map   = mwb_return_new_attribute_map();
        $is_wc_data = $object instanceof WC_Data;
        ...
        if ( $is_wc_data ) {
            ...
            if ( mwb_wc_check_post_columns( $key ) && ( !$value || $object->post->$key == $value ) ) {
                mwb_store_changes( $object->post, $key, '' );
            } else {
                $object_id = is_callable( array( $object, 'get_id' ) ) ? $object->get_id() : $object->id;
...
if ( !function_exists( 'mwb_return_new_attribute_map' ) ) {
    function mwb_return_new_attribute_map() {
        return array(
...
if ( !function_exists( 'mwb_store_changes' ) ) {
    function mwb_store_changes( $object, $key, $value = false ) {
...
if ( !function_exists( 'mwb_send_changes_to_db' ) ) {
    function mwb_send_changes_to_db() {
        global $changed_objects;
        ...
        if ( !empty( $changed_objects ) ) {
            foreach ( $changed_objects as $id => $data ) {
                if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
                    ...
                    mwb_set_prop( $object, $data[ 'changes' ] );
                    $object->save();
                } else {
                    $data[ 'ID' ] = $id;
                    wp_update_post( $data );
...
if ( !function_exists( 'mwb_get_orders' ) ) {
    /**
     *
     */
    function mwb_get_orders( $args ) {
...
if ( !function_exists( 'mwb_get_products' ) ) {
    /**
     *
     */
    function mwb_get_products( $args ) {
        if ( version_compare( WC()->version, '2.7', '<' ) ) {
            $args[ 'fields' ] = 'objects';
            $posts            = get_posts( $args );
...
if ( !function_exists( 'mwb_update_product_stock' ) ) {
    /**
     *
     */
    function mwb_update_product_stock( $product, $stock_quantity = 1, $operation = 'set' ) {
...
if ( !function_exists( 'mwb_wc_deprecated_filters' ) ) {
    /**
     *
     */
    function mwb_wc_deprecated_filters() {
        return apply_filters( 'mwb_wc_deprecated_filters', array(
...
if ( !function_exists( 'mwb_fix_wc_deprecated_filters' ) ) {
    /**
     *
     */
    function mwb_fix_wc_deprecated_filters() {

        if ( !version_compare( WC()->version, '2.7.0', '<' ) ) {
            return;
        }

        $deprecated_filters = mwb_wc_deprecated_filters();
        foreach ( $deprecated_filters as $old => $new ) {
            add_filter( $old, 'mwb_wc_deprecated_filter_mapping', 10, 100 );
        }
    }
}

if ( !function_exists( 'mwb_wc_deprecated_filter_mapping' ) ) {
    /**
     *
     */
    function mwb_wc_deprecated_filter_mapping() {
        $deprecated_filters = mwb_wc_deprecated_filters();
...
if ( !function_exists( 'mwb_wc_check_post_columns' ) ) {
    /**
     *
     */
    function mwb_wc_check_post_columns( $key ) {
        $columns = array(
            'post_author',
            'post_date',
            'post_date_gmt',
...
if ( !function_exists( 'mwb_get_order_id' ) ) {
    /**
     ...
     * @return mixed
     */
    function mwb_get_order_id( $order ) {
        return mwb_get_prop( $order, 'id' );
    }
}

if ( !function_exists( 'mwb_get_product_id' ) ) {
    /**
     ...
     * @return mixed
     */
    function mwb_get_product_id( $product ) {
        return mwb_get_prop( $product, 'id' );
    }
}

if ( !function_exists( 'mwb_get_base_product_id' ) ) {
    /**
     ...
     * @return mixed
     */
    function mwb_get_base_product_id( $product ) {

        return $product instanceof WC_Data && $product->is_type( 'variation' ) ?
            mwb_get_prop( $product, 'parent_id' ) :
            mwb_get_prop( $product, 'id' );
    }
}

if ( !function_exists( 'mwb_get_display_price' ) ) {
    /**
     * @param WC_Product $product
     * @param string     $price
     * @param int        $qty
     */
    function mwb_get_display_price( $product, $price = '', $qty = 1 ) {
...
if ( !function_exists( 'mwb_get_price_excluding_tax' ) ) {
    /**
     * @param WC_Product $product
     * @param int        $qty
     * @param string     $price
     ...
     * @return float|string
     */
    function mwb_get_price_excluding_tax( $product, $qty = 1, $price = '' ) {
...
if ( !function_exists( 'mwb_get_price_including_tax' ) ) {
    /**
     * @param WC_Product $product
     * @param int        $qty
     * @param string     $price
     ...
     * @return float|string
     */
    function mwb_get_price_including_tax( $product, $qty = 1, $price = '' ) {
...
if ( !function_exists( 'mwb_get_product_image_id' ) ) {
    /**
     ...
     */
    function mwb_get_product_image_id( $product, $context = 'view' ) {
...
if ( !function_exists( 'mwb_get_refund_amount' ) ) {
    /**
     ...
     * @return float
     */
    function mwb_get_refund_amount( $refund, $context = 'view' ) {
...
if ( !function_exists( 'mwb_set_refund_amount' ) ){
    /**
     * @param $refund \WC_Order_Refund
     * @param $amount float
     *
     * @return float
     */
    function mwb_set_refund_amount( $refund, $amount ){
        $is_wc_data = $refund instanceof WC_Data;
...
if ( !function_exists( 'mwb_get_refund_reason' ) ){
    /**
     * @param $refund \WC_Order_Refund
     * @param $amount float
     *
     * @return float
     */
    function mwb_get_refund_reason( $refund ){
        $is_wc_data = $refund instanceof WC_Data;
...
if ( !function_exists( 'mwb_product_visibility_meta' ) ) {
    function mwb_product_visibility_meta( $args ) {
        if ( version_compare( WC()->version, '2.7.0', '<' ) ) {
...
if ( !function_exists( 'mwb_datetime_to_timestamp' ) ) {

    /**
     *
     */
    function mwb_datetime_to_timestamp( $date ) {
...
mwb_fix_wc_deprecated_filters();
add_action( 'shutdown', 'mwb_send_changes_to_db' );